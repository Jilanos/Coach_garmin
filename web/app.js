const STORAGE_KEYS={workspace:'coachGarmin.workspace',provider:'coachGarmin.provider',baseUrl:'coachGarmin.baseUrl',apiKey:'coachGarmin.apiKey',sourcePath:'coachGarmin.sourcePath',goalText:'coachGarmin.goalText',injuryText:'coachGarmin.injuryText',fatigueText:'coachGarmin.fatigueText',illnessText:'coachGarmin.illnessText',scheduleText:'coachGarmin.scheduleText',availabilityText:'coachGarmin.availabilityText',temperatureText:'coachGarmin.temperatureText',travelText:'coachGarmin.travelText',otherSportsText:'coachGarmin.otherSportsText',questionText:'coachGarmin.questionText',latestPlanPayload:'coachGarmin.latestPlanPayload',latestQuestionPayload:'coachGarmin.latestQuestionPayload',theme:'coachGarmin.theme',activeSection:'coachGarmin.activeSection',showTerminalMenu:'coachGarmin.showTerminalMenu',startSection:'coachGarmin.startSection',dashboardWindowDays:'coachGarmin.dashboardWindowDays',terminalLevels:'coachGarmin.terminalLevels',terminalLogs:'coachGarmin.terminalLogs',bootId:'coachGarmin.bootId',lastGarminSync:'coachGarmin.lastGarminSync',lastGarminAuthTest:'coachGarmin.lastGarminAuthTest'};
...
const APP_VERSION='20260427-coach32';
...
const sectionMeta={import:{eyebrow:'Import',title:'Import',description:'Commence ici pour voir les données locales, la fraîcheur du workspace et lancer un import ou une sync si besoin.'},dashboard:{eyebrow:'Dashboard',title:'Dashboard',description:'Lis les cartes clés, ouvre une métrique en modal plein écran et regarde les tendances les plus utiles.'},coach:{eyebrow:'Chat',title:'Chat coach',description:'Décris ton objectif, précise tes contraintes, génère un planning puis pose une question ciblée si besoin.'},terminal:{eyebrow:'Terminal',title:'Terminal',description:'Inspecte les logs d’action, de réseau et d’erreur avec un filtre de niveau juste au-dessus.'},settings:{eyebrow:'Settings',title:'Settings',description:'Règle le thème, le provider IA, le workspace local et les options techniques utiles.'}};
const state={workspace:loadStoredText(STORAGE_KEYS.workspace,'data'),provider:loadStoredText(STORAGE_KEYS.provider,'ollama'),baseUrl:loadStoredText(STORAGE_KEYS.baseUrl,''),apiKey:loadStoredText(STORAGE_KEYS.apiKey,''),sourcePath:loadStoredText(STORAGE_KEYS.sourcePath,''),goalText:loadStoredText(STORAGE_KEYS.goalText,''),injuryText:loadStoredText(STORAGE_KEYS.injuryText,''),fatigueText:loadStoredText(STORAGE_KEYS.fatigueText,''),illnessText:loadStoredText(STORAGE_KEYS.illnessText,''),scheduleText:loadStoredText(STORAGE_KEYS.scheduleText,''),availabilityText:loadStoredText(STORAGE_KEYS.availabilityText,''),temperatureText:loadStoredText(STORAGE_KEYS.temperatureText,''),travelText:loadStoredText(STORAGE_KEYS.travelText,''),otherSportsText:loadStoredText(STORAGE_KEYS.otherSportsText,''),questionText:loadStoredText(STORAGE_KEYS.questionText,''),latestPlanPayload:loadStoredObject(STORAGE_KEYS.latestPlanPayload,null),latestQuestionPayload:loadStoredObject(STORAGE_KEYS.latestQuestionPayload,null),theme:loadStoredText(STORAGE_KEYS.theme,'data-lab'),activeSection:loadStoredText(STORAGE_KEYS.activeSection,'import'),showTerminalMenu:loadStoredText(STORAGE_KEYS.showTerminalMenu,'true')!=='false',startSection:loadStoredText(STORAGE_KEYS.startSection,'import'),dashboardWindowDays:normalizeDashboardWindowDays(loadStoredText(STORAGE_KEYS.dashboardWindowDays,'90')),openTrendKey:null,chartModes:{sleep:'raw',hrv:'raw'},terminalLevels:loadTerminalLevels(),terminalLogs:loadTerminalLogs(),installPrompt:null,dashboardPayload:null,retryAction:null,bootTrace:[],bootErrors:[],lastGarminSync:loadLastGarminSync(),lastGarminAuthTest:loadLastGarminAuthTest(),bootId:loadStoredText(STORAGE_KEYS.bootId,String(Date.now())),profileSaveTimer:null,profileSaveInFlight:false,profileSaveQueued:false,lastProfileSaveSignature:''};
const $=(id)=>document.getElementById(id);
const dom={appChip:$('app-chip'),providerChip:$('provider-chip'),dataChip:$('data-chip'),freshnessChip:$('freshness-chip'),objectiveChip:$('objective-chip'),workspaceChip:$('workspace-chip'),buildChip:$('build-chip'),sidebarNav:$('sidebar-nav'),sectionEyebrow:$('section-eyebrow'),sectionTitle:$('section-title'),sectionDescription:$('section-description'),sectionBadges:$('section-badges'),installButton:$('install-button'),diagnosticButton:$('diagnostic-button'),coachSummary:$('coach-summary'),planList:$('plan-list'),planMeta:$('plan-meta'),questionResponse:$('question-response'),questionMeta:$('question-meta'),busyBanner:$('busy-banner'),busyText:$('busy-text'),coachErrorBanner:$('coach-error-banner'),coachErrorTitle:$('coach-error-title'),coachErrorMessage:$('coach-error-message'),retryButton:$('retry-button'),dashboardCards:$('dashboard-cards'),dashboardImportState:$('dashboard-import-state'),dashboardImportDetail:$('dashboard-import-detail'),dashboardBikeState:$('dashboard-bike-state'),dashboardBikeDetail:$('dashboard-bike-detail'),dashboardHrvState:$('dashboard-hrv-state'),dashboardHrvDetail:$('dashboard-hrv-detail'),coverageCard:$('coverage-card'),coverageSubtitle:$('coverage-subtitle'),analysisCard:$('analysis-card'),analysisSubtitle:$('analysis-subtitle'),volumeTrendSummary:$('volume-trend-summary'),bikeVolumeTrendSummary:$('bike-volume-trend-summary'),loadRatioTrendSummary:$('load-ratio-trend-summary'),loadTrendSummary:$('load-trend-summary'),sleepTrendSummary:$('sleep-trend-summary'),restingHrTrendSummary:$('resting-hr-trend-summary'),hrvTrendSummary:$('hrv-trend-summary'),paceHrTrendSummary:$('pace-hr-trend-summary'),cadenceTrendSummary:$('cadence-trend-summary'),paceCadenceHrTrendSummary:$('pace-cadence-hr-trend-summary'),sessionTypesTrendSummary:$('session-types-trend-summary'),zonesAllSummary:$('zones-all-summary'),zonesRunningSummary:$('zones-running-summary'),volumeTrend:$('volume-trend'),bikeVolumeTrend:$('bike-volume-trend'),loadRatioTrend:$('load-ratio-trend'),loadTrend:$('load-trend'),sleepTrend:$('sleep-trend'),restingHrTrend:$('resting-hr-trend'),hrvTrend:$('hrv-trend'),paceHrTrend:$('pace-hr-trend'),cadenceTrend:$('cadence-trend'),paceCadenceHrTrend:$('pace-cadence-hr-trend'),sessionTypesTrend:$('session-types-trend'),zonesAllChart:$('zones-all-chart'),zonesRunningChart:$('zones-running-chart'),importDataState:$('import-data-state'),importFreshnessDetail:$('import-freshness-detail'),importWorkspace:$('import-workspace'),importWorkspaceDetail:$('import-workspace-detail'),importRunState:$('import-run-state'),importRunDetail:$('import-run-detail'),importAgeState:$('import-age-state'),importAgeDetail:$('import-age-detail'),importSyncState:$('import-sync-state'),importSyncDetail:$('import-sync-detail'),coachStatusStrip:$('coach-status-strip'),terminalLog:$('terminal-log'),terminalSummary:$('terminal-summary'),dashboardModal:$('dashboard-modal'),dashboardModalTitle:$('dashboard-modal-title'),dashboardModalSubtitle:$('dashboard-modal-subtitle'),dashboardModalValue:$('dashboard-modal-value'),dashboardModalText:$('dashboard-modal-text'),dashboardModalChart:$('dashboard-modal-chart'),dashboardModalSignals:$('dashboard-modal-signals'),dashboardModalClose:$('dashboard-modal-close'),diagnosticModal:$('diagnostic-modal'),diagnosticModalTitle:$('diagnostic-modal-title'),diagnosticModalSubtitle:$('diagnostic-modal-subtitle'),diagnosticModalClose:$('diagnostic-modal-close'),diagnosticBuild:$('diagnostic-build'),diagnosticBuildDetail:$('diagnostic-build-detail'),diagnosticUrl:$('diagnostic-url'),diagnosticUrlDetail:$('diagnostic-url-detail'),diagnosticSw:$('diagnostic-sw'),diagnosticSwDetail:$('diagnostic-sw-detail'),diagnosticWorkspace:$('diagnostic-workspace'),diagnosticWorkspaceDetail:$('diagnostic-workspace-detail'),diagnosticImport:$('diagnostic-import'),diagnosticImportDetail:$('diagnostic-import-detail'),diagnosticProvider:$('diagnostic-provider'),diagnosticProviderDetail:$('diagnostic-provider-detail'),diagnosticRefreshButton:$('diagnostic-refresh-button'),diagnosticCopyButton:$('diagnostic-copy-button'),diagnosticSummary:$('diagnostic-summary'),bootTraceState:$('boot-trace-state'),bootTraceDetail:$('boot-trace-detail'),bootTraceLog:$('boot-trace-log'),bootTraceRefreshButton:$('boot-trace-refresh-button'),debugEndpointState:$('debug-endpoint-state'),debugEndpointDetail:$('debug-endpoint-detail'),terminalClearButton:$('clear-terminal-button'),themeSelect:$('theme-select'),startSectionSelect:$('start-section-select'),showTerminalToggle:$('show-terminal-toggle'),settingsProviderSelect:$('settings-provider-select'),baseUrlInput:$('base-url-input'),apiKeyInput:$('api-key-input'),workspaceInput:$('workspace-input'),workspaceInputSettings:$('workspace-input-settings'),sourcePathInput:$('source-path-input'),sourcePathInputSettings:$('source-path-input-settings'),goalInput:$('goal-input'),injuryInput:$('injury-input'),fatigueInput:$('fatigue-input'),illnessInput:$('illness-input'),scheduleInput:$('schedule-input'),availabilityInput:$('availability-input'),temperatureInput:$('temperature-input'),travelInput:$('travel-input'),otherSportsInput:$('other-sports-input'),questionInput:$('question-input'),coachWorkspaceInput:$('coach-workspace-chip-input'),authDebugState:$('auth-debug-state'),authDebugDetail:$('auth-debug-detail'),authTokenstoreState:$('auth-tokenstore-state'),authTokenstoreDetail:$('auth-tokenstore-detail'),authTestButton:$('auth-test-button'),providerSelect:$('provider-select'),saveSettingsButton:$('save-settings-button'),refreshSettingsButton:$('refresh-settings-button'),planButton:$('plan-button'),askQuestionButton:$('ask-question-button'),importButton:$('import-button'),syncButton:$('sync-button'),refreshButton:$('refresh-button'),reprocessButton:$('reprocess-button'),useLastWorkspaceButton:$('use-last-workspace-button'),levelButtons:Array.from(document.querySelectorAll('.level-toggle')),navButtons:Array.from(document.querySelectorAll('.nav-button'))};
const actionButtons=[dom.saveSettingsButton,dom.refreshSettingsButton,dom.planButton,dom.askQuestionButton,dom.importButton,dom.syncButton,dom.refreshButton,dom.reprocessButton,dom.useLastWorkspaceButton,dom.retryButton,dom.terminalClearButton,dom.authTestButton];
...
function renderBootTrace(events=state.bootTrace){
  ...
  if(dom.bootTraceDetail){
    ...
    dom.bootTraceDetail.textContent=last?formatBootTraceEvent(last):'Aucun événement enregistré.';
  }
  if(dom.bootTraceLog){
...
function syncInputsFromState(){if(dom.workspaceInputSettings) dom.workspaceInputSettings.value=state.workspace;if(dom.providerSelect) dom.providerSelect.value=state.provider;if(dom.settingsProviderSelect) dom.settingsProviderSelect.value=state.provider;if(dom.baseUrlInput) dom.baseUrlInput.value=state.baseUrl;if(dom.apiKeyInput) dom.apiKeyInput.value=state.apiKey;if(dom.sourcePathInput) dom.sourcePathInput.value=state.sourcePath;if(dom.sourcePathInputSettings) dom.sourcePathInputSettings.value=state.sourcePath;if(dom.goalInput) dom.goalInput.value=state.goalText;if(dom.injuryInput) dom.injuryInput.value=state.injuryText;if(dom.fatigueInput) dom.fatigueInput.value=state.fatigueText;if(dom.illnessInput) dom.illnessInput.value=state.illnessText;if(dom.scheduleInput) dom.scheduleInput.value=state.scheduleText;if(dom.availabilityInput) dom.availabilityInput.value=state.availabilityText;if(dom.temperatureInput) dom.temperatureInput.value=state.temperatureText;if(dom.travelInput) dom.travelInput.value=state.travelText;if(dom.otherSportsInput) dom.otherSportsInput.value=state.otherSportsText;if(dom.questionInput) dom.questionInput.value=state.questionText;if(dom.themeSelect) dom.themeSelect.value=state.theme;if(dom.startSectionSelect) dom.startSectionSelect.value=state.startSection;if(dom.showTerminalToggle) dom.showTerminalToggle.checked=state.showTerminalMenu;if(dom.coachWorkspaceInput) dom.coachWorkspaceInput.value=state.workspace}
function persistSettings(){state.workspace=normalizeText((dom.workspaceInputSettings?.value||'data').trim()||'data');state.provider=normalizeText((dom.providerSelect?.value||dom.settingsProviderSelect?.value||'ollama').trim());state.baseUrl=normalizeText((dom.baseUrlInput?.value||'').trim());state.apiKey=normalizeText((dom.apiKeyInput?.value||'').trim());state.sourcePath=normalizeText((dom.sourcePathInput?.value||dom.sourcePathInputSettings?.value||'').trim());state.goalText=normalizeText((dom.goalInput?.value||'').trim());state.injuryText=normalizeText((dom.injuryInput?.value||'').trim());state.fatigueText=normalizeText((dom.fatigueInput?.value||'').trim());state.illnessText=normalizeText((dom.illnessInput?.value||'').trim());state.scheduleText=normalizeText((dom.scheduleInput?.value||'').trim());state.availabilityText=normalizeText((dom.availabilityInput?.value||'').trim());state.temperatureText=normalizeText((dom.temperatureInput?.value||'').trim());state.travelText=normalizeText((dom.travelInput?.value||'').trim());state.otherSportsText=normalizeText((dom.otherSportsInput?.value||'').trim());state.questionText=normalizeText((dom.questionInput?.value||'').trim());state.theme=dom.themeSelect?.value||state.theme;state.startSection=dom.startSectionSelect?.value||state.startSection;state.dashboardWindowDays=normalizeDashboardWindowDays(state.dashboardWindowDays||90);state.showTerminalMenu=!!dom.showTerminalToggle?.checked;localStorage.setItem(STORAGE_KEYS.workspace,state.workspace);localStorage.setItem(STORAGE_KEYS.provider,state.provider);localStorage.setItem(STORAGE_KEYS.baseUrl,state.baseUrl);localStorage.setItem(STORAGE_KEYS.apiKey,state.apiKey);localStorage.setItem(STORAGE_KEYS.sourcePath,state.sourcePath);localStorage.setItem(STORAGE_KEYS.goalText,state.goalText);localStorage.setItem(STORAGE_KEYS.injuryText,state.injuryText);localStorage.setItem(STORAGE_KEYS.fatigueText,state.fatigueText);localStorage.setItem(STORAGE_KEYS.illnessText,state.illnessText);localStorage.setItem(STORAGE_KEYS.scheduleText,state.scheduleText);localStorage.setItem(STORAGE_KEYS.availabilityText,state.availabilityText);localStorage.setItem(STORAGE_KEYS.temperatureText,state.temperatureText);localStorage.setItem(STORAGE_KEYS.travelText,state.travelText);localStorage.setItem(STORAGE_KEYS.otherSportsText,state.otherSportsText);localStorage.setItem(STORAGE_KEYS.questionText,state.questionText);localStorage.setItem(STORAGE_KEYS.theme,state.theme);localStorage.setItem(STORAGE_KEYS.activeSection,state.activeSection);localStorage.setItem(STORAGE_KEYS.startSection,state.startSection);localStorage.setItem(STORAGE_KEYS.dashboardWindowDays,String(state.dashboardWindowDays));localStorage.setItem(STORAGE_KEYS.showTerminalMenu,String(state.showTerminalMenu));localStorage.setItem(STORAGE_KEYS.terminalLevels,JSON.stringify(state.terminalLevels));applyTheme();updateNavVisibility();renderSectionHeader();renderSidebarStatus();if(dom.coachWorkspaceInput) dom.coachWorkspaceInput.value=state.workspace}
function textOrNull(value){const text=normalizeText(String(value||'').trim());return text||null}
function buildCoachProfilePayload(){return{goal_text:textOrNull(state.goalText),blessure:textOrNull(state.injuryText),fatigue:textOrNull(state.fatigueText),maladie:textOrNull(state.illnessText),emploi_du_temps:textOrNull(state.scheduleText),disponibilite:textOrNull(state.availabilityText),temperature:textOrNull(state.temperatureText),deplacements:textOrNull(state.travelText),autres_sports:textOrNull(state.otherSportsText),targeted_question:textOrNull(state.questionText)}}
function applyCoachProfile(goalProfile,{preferExisting=true}={}){if(!goalProfile||typeof goalProfile!=='object')return;const mapping=[['goalText','goal_text'],['injuryText','blessure'],['fatigueText','fatigue'],['illnessText','maladie'],['scheduleText','emploi_du_temps'],['availabilityText','disponibilite'],['temperatureText','temperature'],['travelText','deplacements'],['otherSportsText','autres_sports'],['questionText','targeted_question']];mapping.forEach(([stateKey,payloadKey])=>{const current=state[stateKey];const incoming=normalizeText((goalProfile[payloadKey]??'').toString().trim());if(preferExisting&&current)return;if(incoming)state[stateKey]=incoming});syncInputsFromState();persistSettings()}
function buildCoachProfileRequestBody(){const goalText=normalizeText((dom.goalInput?.value||'').trim());return{goal_text:goalText,data_dir:state.workspace,provider:state.provider,base_url:state.baseUrl||null,api_key:state.apiKey||null,profile:buildCoachProfilePayload()}}
function buildCoachProfileSignature(body){return JSON.stringify({goal_text:body.goal_text,data_dir:body.data_dir,provider:body.provider,base_url:body.base_url||null,profile:body.profile||{}})}
async function syncCoachProfile({force=false,quiet=true}={}){persistSettings();const body=buildCoachProfileRequestBody();if(!body.goal_text)return null;const signature=buildCoachProfileSignature(body);if(!force&&signature===state.lastProfileSaveSignature)return null;if(state.profileSaveInFlight){state.profileSaveQueued=true;return null}state.profileSaveInFlight=true;try{const payload=await requestJson('/api/coach/profile',{method:'POST',body:JSON.stringify(body)},'coach-profile');if(payload.goal_profile)applyCoachProfile(payload.goal_profile,{preferExisting:false});state.dashboardPayload=payload.dashboard||state.dashboardPayload;state.lastProfileSaveSignature=signature;renderAllSections();if(!quiet)addTerminalLog('info','coach','Contexte coaching synchronisé',payload.goal_profile_path||'profil local mis à jour');return payload}catch(error){addTerminalLog('warn','coach','Synchronisation du contexte indisponible',error.message);return null}finally{state.profileSaveInFlight=false;if(state.profileSaveQueued){state.profileSaveQueued=false;queueMicrotask(()=>{syncCoachProfile({force:false,quiet:true}).catch(()=>{})})}}}
function scheduleCoachProfileSync(){persistSettings();renderSidebarStatus();if(dom.coachWorkspaceInput) dom.coachWorkspaceInput.value=state.workspace;if(state.profileSaveTimer){clearTimeout(state.profileSaveTimer);state.profileSaveTimer=null}if(!state.goalText)return;state.profileSaveTimer=window.setTimeout(()=>{state.profileSaveTimer=null;syncCoachProfile({force:false,quiet:true}).catch(()=>{})},450)}
async function flushCoachProfileSync({force=false,quiet=true}={}){if(state.profileSaveTimer){clearTimeout(state.profileSaveTimer);state.profileSaveTimer=null}return syncCoachProfile({force,quiet})}
...
function addMessage(role,text){if(!dom.transcript)return;const node=document.createElement('div');node.className=`message ${role}`;node.textContent=text;dom.transcript.appendChild(node);dom.transcript.scrollTop=dom.transcript.scrollHeight}
function renderCoachMeta(container,label,lines=[]){if(!container)return;container.innerHTML='';const items=(Array.isArray(lines)?lines:[]).map((line)=>normalizeText(String(line||'').trim())).filter(Boolean);if(!items.length)return;const wrapper=document.createElement('span');wrapper.className='coach-info';const chip=document.createElement('span');chip.className='coach-info-chip';chip.tabIndex=0;chip.textContent=label;const panel=document.createElement('div');panel.className='coach-info-panel';panel.setAttribute('role','note');items.forEach((line)=>{const row=document.createElement('p');row.textContent=line;panel.appendChild(row)});wrapper.append(chip,panel);container.appendChild(wrapper)}
function workspaceScopedPayload(payload){return payload&&typeof payload==='object'&&payload.workspace===state.workspace?payload:null}
function persistWorkspacePayload(storageKey,payload){if(!payload){localStorage.removeItem(storageKey);return}localStorage.setItem(storageKey,JSON.stringify(payload))}
function storeLatestPlanPayload(payload){const snapshot={workspace:state.workspace,coach_summary:payload?.coach_summary||'',signals_used:Array.isArray(payload?.signals_used)?payload.signals_used:[],weekly_plan:Array.isArray(payload?.weekly_plan)?payload.weekly_plan:[],analysis:payload?.analysis&&typeof payload.analysis==='object'?payload.analysis:{},generated_at:payload?.generated_at||new Date().toISOString()};state.latestPlanPayload=snapshot;persistWorkspacePayload(STORAGE_KEYS.latestPlanPayload,snapshot)}
function storeLatestQuestionPayload(payload){const snapshot={workspace:state.workspace,question_text:payload?.question_text||'',coach_answer:payload?.coach_answer||'',follow_up:payload?.follow_up||'',signals_used:Array.isArray(payload?.signals_used)?payload.signals_used:[],plan_context_note:payload?.plan_context_note||'',saved_at:new Date().toISOString()};state.latestQuestionPayload=snapshot;persistWorkspacePayload(STORAGE_KEYS.latestQuestionPayload,snapshot)}
function clearLatestPlanPayload(){state.latestPlanPayload=null;persistWorkspacePayload(STORAGE_KEYS.latestPlanPayload,null)}
function renderEmptyPlanState(message='Aucun plan généré pour le moment.'){if(dom.coachSummary)dom.coachSummary.textContent=message;if(dom.planList)dom.planList.innerHTML='';renderCoachMeta(dom.planMeta,'Infos utilisées',[])}
function renderEmptyQuestionState(message='Aucune question posée pour le moment.'){if(dom.questionResponse)dom.questionResponse.textContent=message;renderCoachMeta(dom.questionMeta,'Contexte',[])}
function hydrateCoachOutputs(){const latestPlan=workspaceScopedPayload(state.latestPlanPayload);if(latestPlan&&Array.isArray(latestPlan.weekly_plan)&&latestPlan.weekly_plan.length){renderSummary(latestPlan)}else{renderEmptyPlanState()}const latestQuestion=workspaceScopedPayload(state.latestQuestionPayload);if(latestQuestion&&(latestQuestion.coach_answer||latestQuestion.follow_up||latestQuestion.question_text)){renderQuestionResponse(latestQuestion)}else{renderEmptyQuestionState()}}
function renderQuestions(){return}
...
}function renderBarChart(container,series,options={}){const input=Array.isArray(series)?series:[];const items=input.map((entry,index)=>{if(!entry||typeof entry!=='object')return null;const value=Number(entry.value??entry.distance_km??entry.load_ratio_7_28??entry.sleep_hours??entry.resting_hr??entry.cadence_spm??entry.hrv_ms??entry.pace_min_per_km??entry.heart_rate);return Number.isFinite(value)?{value,label:entry.label||entry.metric_date||String(index+1),detail:entry.detail||entry.metric_date||''}:null}).filter(Boolean);if(!items.length){renderSparkline(container,series,options);return}const width=options.width||360;const height=options.height||140;const padLeft=46;const padRight=18;const padTop=12;const padBottom=34;const values=items.map((item)=>item.value);const domain=buildChartDomain([0,...values],[],{padRatio:0.06,minSpan:1});const min=Math.min(0,domain.min);const max=domain.max;const span=max-min||1;const barWidth=Math.max(10,((width-padLeft-padRight)/items.length)-6);const step=(width-padLeft-padRight)/items.length;const yTicks=buildChartTicks(min,max,5);const xTicks=buildIndexedTicks(items.length,options.xTickCount||(width>=900?7:5));const tickFormatter=options.tickFormatter||((value)=>formatNumber(value,1));container.innerHTML=`<div class="chart-shell"><svg class="chart-svg" viewBox="0 0 ${width} ${height}" preserveAspectRatio="none" role="img" aria-label="${safeChartText(options.title||'histogramme')}"><line class="chart-axis-line" x1="${padLeft}" y1="${height-padBottom}" x2="${width-padRight}" y2="${height-padBottom}"></line><line class="chart-axis-line" x1="${padLeft}" y1="${padTop}" x2="${padLeft}" y2="${height-padBottom}"></line>${yTicks.map((tick)=>{const y=height-padBottom-((tick-min)/span)*(height-padTop-padBottom);return `<g><line class="chart-grid-line" x1="${padLeft}" y1="${y.toFixed(2)}" x2="${width-padRight}" y2="${y.toFixed(2)}"></line><text class="chart-axis-tick" x="${padLeft-8}" y="${(y+4).toFixed(2)}" text-anchor="end">${safeChartText(tickFormatter(tick))}</text></g>`}).join('')}${items.map((item,index)=>{const x=padLeft+(index*step)+((step-barWidth)/2);const y=height-padBottom-((item.value-min)/span)*(height-padTop-padBottom);const h=Math.max((height-padBottom)-y,1.5);return `<rect class="chart-bar" x="${x.toFixed(2)}" y="${y.toFixed(2)}" width="${barWidth.toFixed(2)}" height="${h.toFixed(2)}" rx="4" fill="${options.stroke||'#8ae6c0'}" fill-opacity="0.82" data-title="${safeChartText(item.label)}" data-value="${safeChartText(tickFormatter(item.value))}" data-detail="${safeChartText(item.detail||'')}" data-tx="${Math.max(8,Math.min(width-180,x+barWidth+8)).toFixed(2)}" data-ty="${Math.max(18,y-10).toFixed(2)}"></rect><rect class="chart-point-hit" x="${x.toFixed(2)}" y="${Math.min(y,height-padBottom).toFixed(2)}" width="${barWidth.toFixed(2)}" height="${Math.max(h,16).toFixed(2)}" fill="transparent" data-title="${safeChartText(item.label)}" data-value="${safeChartText(tickFormatter(item.value))}" data-detail="${safeChartText(item.detail||'')}" data-tx="${Math.max(8,Math.min(width-180,x+barWidth+8)).toFixed(2)}" data-ty="${Math.max(18,y-10).toFixed(2)}"></rect>`}).join('')}${xTicks.map((index)=>{const x=padLeft+(index*step)+(step/2);return `<text class="chart-axis-tick" x="${x.toFixed(2)}" y="${height-8}" text-anchor="middle">${safeChartText(items[index]?.label||'')}</text>`}).join('')}</svg><div class="chart-tooltip" aria-hidden="true"></div></div>`;const tooltip=chartTooltipLabel(container);attachChartTooltipHandlers(container,tooltip,Array.from(container.querySelectorAll('.chart-bar, .chart-point-hit')))}
...
function startOfIsoWeek(date){const value=new Date(date);value.setHours(0,0,0,0);const weekday=(value.getDay()+6)%7;value.setDate(value.getDate()-weekday);return value}
function formatIsoWeekLabel(date){const start=startOfIsoWeek(date);const end=new Date(start);end.setDate(end.getDate()+6);return `${formatShortDateLabel(start.toISOString().slice(0,10))} - ${formatShortDateLabel(end.toISOString().slice(0,10))}`}
function aggregateSeriesByWindow(rows,days,{valueKey='value',mode='sum'}={}){const series=windowedChartSeries(rows,days,{valueKey,labelMode:'date'});if(!series.length)return[];if(normalizeDashboardWindowDays(days)<365)return series;const buckets=new Map();series.forEach((item)=>{const parsed=parseSeriesDate(item.metric_date||item.date);if(!(parsed instanceof Date))return;const start=startOfIsoWeek(parsed);const bucketKey=start.toISOString().slice(0,10);const current=buckets.get(bucketKey)||{metric_date:bucketKey,label:formatIsoWeekLabel(parsed),detail:`Semaine du ${formatDateLabel(bucketKey)}`,values:[],sources:[]};current.values.push(Number(item.value));current.sources.push(item);buckets.set(bucketKey,current)});return Array.from(buckets.values()).sort((a,b)=>String(a.metric_date).localeCompare(String(b.metric_date))).map((bucket)=>{const values=bucket.values.filter((value)=>Number.isFinite(value));const aggregated=mode==='average'?(values.reduce((sum,value)=>sum+value,0)/(values.length||1)):values.reduce((sum,value)=>sum+value,0);return{metric_date:bucket.metric_date,label:bucket.label,detail:bucket.detail,value:Number(aggregated.toFixed(mode==='average'?1:2)),source:bucket.sources}})}
...
function buildSessionTypeDistribution(rows){
  const labels={easy_jog:'Jogg facile',quality:'Qualité',long_run:'Sortie longue'};
  const order=['easy_jog','quality','long_run'];
  const input=Array.isArray(rows)?rows:[];
  const total=input.length||0;
  return order.map((key)=>{const matches=input.filter((item)=>item&&item.session_type===key);const count=matches.length;const totalLoad=matches.reduce((sum,item)=>sum+Number(item.training_load||0),0);return{session_type:key,label:labels[key],value:count,detail:count?`${count} séance${count>1?'s':''} · ${formatNumber((count/Math.max(total,1))*100,0)}% · charge ${formatNumber(totalLoad,0)}`:'0 séance',count,training_load:roundNumber(totalLoad,1)}})
}
function roundNumber(value,digits=1){const numeric=Number(value);if(!Number.isFinite(numeric))return 0;return Number(numeric.toFixed(digits))}
function buildTrendWindowSeries(trend,days){const sleepMode=state.chartModes?.sleep||'raw';const hrvMode=state.chartModes?.hrv||'raw';const sessionTypes=Array.isArray(trend.running_session_types)?trend.running_session_types:[];return{runningSeries:aggregateSeriesByWindow(trend.daily_volume,days,{valueKey:'distance_km',mode:'sum'}).map((row)=>({...row,distance_km:row.value})),bikeSeries:aggregateSeriesByWindow(trend.daily_bike_volume,days,{valueKey:'distance_km',mode:'sum'}).map((row)=>({...row,distance_km:row.value})),loadSeries:windowedChartSeries(trend.daily_load,days,{valueKey:'activity_load'}),loadRatioSeries:windowedChartSeries(trend.daily_load_ratio,days,{valueKey:'load_ratio_7_28'}),sleepSeriesRaw:windowedChartSeries(trend.daily_sleep,days,{valueKey:'sleep_hours'}),sleepSeriesSmoothed:windowedChartSeries(trend.daily_sleep_smoothed,days,{valueKey:'sleep_hours'}),sleepSeries:windowedChartSeries(sleepMode==='smooth'?trend.daily_sleep_smoothed:trend.daily_sleep,days,{valueKey:'sleep_hours'}),restingHrSeries:windowedChartSeries(trend.daily_resting_hr,days,{valueKey:'resting_hr'}),hrvSeriesRaw:windowedChartSeries(trend.daily_hrv,days,{valueKey:'hrv_ms'}),hrvSeriesSmoothed:windowedChartSeries(trend.daily_hrv_smoothed,days,{valueKey:'hrv_ms'}),hrvSeries:windowedChartSeries(hrvMode==='smooth'?trend.daily_hrv_smoothed:trend.daily_hrv,days,{valueKey:'hrv_ms'}),cadenceSeries:windowedChartSeries(trend.cadence_daily,days,{valueKey:'cadence_spm'}),cadenceDiagnostics:trend.cadence_diagnostics||{activities:[],summary:{}},runningPaceSeries:windowedChartSeries(trend.daily_running_pace,days,{valueKey:'pace_min_per_km'}),runningHrSeries:windowedChartSeries(trend.daily_running_hr,days,{valueKey:'heart_rate'}),paceCurve:Array.isArray(trend.pace_hr_curve)?trend.pace_hr_curve:[],paceCurveDebug:trend.pace_hr_curve_debug||{},sessionTypeRows:sessionTypes,sessionTypeDistribution:buildSessionTypeDistribution(sessionTypes)}}
...
function renderDetailBlocks(container,card){
  ...
  const normalize=(distribution)=>zones.map((zone)=>{
    ...
    return {zone:zone.label,color:zone.color,minutes:Number(match?.minutes||0),seconds:Number(match?.seconds||0),share:Number(match?.share||0),bpmRange:match?.bpm_range||'bpm non défini'};
  });
  const modes={all:{label:options.overallLabel||'Toutes activités',dataset:overall||{distribution:[]}},running:{label:options.runningLabel||'Running',dataset:running||{distribution:[]}}};
  const renderMode=(modeKey='all')=>{
    const mode=modes[modeKey]||modes.all;
    const distribution=normalize(mode.dataset?.distribution||[]);
    const totalSeconds=distribution.reduce((sum,entry)=>sum+Number(entry.seconds||0),0);
    const total=Math.max(totalSeconds,1);
    const radius=68;
    const cx=95;
    const cy=95;
    let offset=25;
    const segments=distribution.map((entry)=>{const percent=Math.max(Number(entry.seconds||0)/total,0);const length=percent*360;const segment=`<circle cx="${cx}" cy="${cy}" r="${radius}" fill="none" stroke="${entry.color}" stroke-width="18" stroke-linecap="round" stroke-dasharray="${length.toFixed(3)} ${Math.max(360-length,0.001).toFixed(3)}" stroke-dashoffset="${offset.toFixed(3)}" transform="rotate(-90 ${cx} ${cy})"></circle>`;offset-=length;return segment;}).join('');
    container.innerHTML=`<div class="zone-comparison"><div class="zone-switch"><button type="button" class="zone-mode-button${modeKey==='all'?' active':''}" data-zone-mode="all">Toutes activités</button><button type="button" class="zone-mode-button${modeKey==='running'?' active':''}" data-zone-mode="running">Running</button></div><div class="zone-comparison-grid"><div class="zone-donut"><svg viewBox="0 0 190 190" aria-label="${safeChartText(mode.label)}"><circle cx="${cx}" cy="${cy}" r="${radius}" fill="none" stroke="rgba(255,255,255,0.08)" stroke-width="18"></circle>${segments}<circle cx="${cx}" cy="${cy}" r="48" fill="rgba(6, 10, 18, 0.88)"></circle><text x="95" y="90" text-anchor="middle" class="chart-axis-title">${safeChartText(mode.label)}</text><text x="95" y="109" text-anchor="middle" class="chart-note">${formatNumber(totalSeconds/3600,1)} h</text></svg><div class="zone-legend">${distribution.map((entry)=>`<div class="zone-legend-row"><span><i style="background:${entry.color};width:10px;height:10px;border-radius:999px;display:inline-block;margin-right:8px"></i>${entry.zone} · ${safeChartText(entry.bpmRange)}</span><span>${formatNumber(entry.minutes,1)} min · ${formatNumber((entry.share||0)*100,0)}%</span></div>`).join('')}</div></div></div><p class="status-line">${safeChartText(options.note||'Répartition approximative du temps passé par zones FC à partir de la FC moyenne des activités.')}</p></div>`;
    Array.from(container.querySelectorAll('[data-zone-mode]')).forEach((button)=>{button.addEventListener('click',()=>renderMode(button.dataset.zoneMode));});
  };
  renderMode(options.defaultMode||'all');
}
function renderTriptychChart(container,paceSeries,cadenceSeries,hrSeries,options={}){
  const paceItems=Array.isArray(paceSeries)?paceSeries.filter((item)=>item&&Number.isFinite(Number(item.value??item.pace_min_per_km))):[];
  ...
  if(!paceItems.length&&!cadenceItems.length&&!hrItems.length){
    ...
    const emptyReason=safeChartText(options.emptyReason||'Les trois séries n’ont pas encore de points exploitables.');
    const emptyNotes=Array.isArray(options.emptyNotes)?options.emptyNotes.filter(Boolean):[];
  ...
  const paceColor='#8ae6c0';
  const cadenceColor='#8ab4ff';
  const hrColor='#ffbe55';
  const keyFor=(item,index)=>item.metric_date||item.date||item.label||String(index);
  const mergeSeries=(items,valueResolver)=>{const map=new Map();items.forEach((item,index)=>{map.set(keyFor(item,index),{label:item.label||formatShortDateLabel(item.metric_date||item.date||''),detail:item.detail||formatDateLabel(item.metric_date||item.date||''),value:Number(valueResolver(item))});});return map;};
  const paceMap=mergeSeries(paceItems,(item)=>item.value??item.pace_min_per_km);
  const cadenceMap=mergeSeries(cadenceItems,(item)=>item.value??item.cadence_spm);
  const hrMap=mergeSeries(hrItems,(item)=>item.value??item.heart_rate);
  const keys=Array.from(new Set([...paceMap.keys(),...cadenceMap.keys(),...hrMap.keys()])).sort((a,b)=>{const left=parseSeriesDate(a);const right=parseSeriesDate(b);if(left instanceof Date&&right instanceof Date)return left-right;return String(a).localeCompare(String(b));});
  const merged=keys.map((key)=>({key,label:paceMap.get(key)?.label||cadenceMap.get(key)?.label||hrMap.get(key)?.label||key,detail:paceMap.get(key)?.detail||cadenceMap.get(key)?.detail||hrMap.get(key)?.detail||'',pace:paceMap.get(key)?.value,cadence:cadenceMap.get(key)?.value,hr:hrMap.get(key)?.value}));
  const paceValues=merged.map((item)=>item.pace).filter((value)=>Number.isFinite(value));
  const cadenceValues=merged.map((item)=>item.cadence).filter((value)=>Number.isFinite(value));
  const hrValues=merged.map((item)=>item.hr).filter((value)=>Number.isFinite(value));
  const paceDomain=buildChartDomain(paceValues,(options.paceReferenceLines||[]).map((entry)=>Number(entry.value)),{padRatio:0.05,minSpan:0.2});
  const cadenceDomain=buildChartDomain(cadenceValues,(options.cadenceReferenceLines||[]).map((entry)=>Number(entry.value)),{padRatio:0.05,minSpan:4});
  const hrDomain=buildChartDomain(hrValues,[...(options.hrReferenceLines||[]).map((entry)=>Number(entry.value)),...(options.hrReferenceBands||[]).flatMap((entry)=>[Number(entry.low),Number(entry.high)])],{padRatio:0.05,minSpan:6});
  const innerWidth=width-padLeft-padRight;
  const innerHeight=height-padTop-padBottom;
  const step=merged.length>1?innerWidth/(merged.length-1):0;
  const paceY=(value)=>height-padBottom-((Number(value)-paceDomain.min)/(paceDomain.span||1))*innerHeight;
  const cadenceY=(value)=>height-padBottom-((Number(value)-cadenceDomain.min)/(cadenceDomain.span||1))*innerHeight;
  const hrY=(value)=>height-padBottom-((Number(value)-hrDomain.min)/(hrDomain.span||1))*innerHeight;
  const buildLine=(field,color,yFn,formatter)=>{const points=merged.map((item,index)=>Number.isFinite(item[field])?{x:merged.length>1?padLeft+index*step:padLeft+innerWidth/2,y:yFn(item[field]),item}:null).filter(Boolean);return{path:points.length?`<path d="M ${points.map((point)=>`${point.x.toFixed(2)} ${point.y.toFixed(2)}`).join(' L ')}" fill="none" stroke="${color}" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"></path>`:'',hits:points.map((point)=>`<circle class="chart-point-dot" cx="${point.x.toFixed(2)}" cy="${point.y.toFixed(2)}" r="3.8" fill="${color}" data-title="${safeChartText(point.item.label)}" data-value="${safeChartText(formatter(point.item[field]))}" data-detail="${safeChartText(point.item.detail)}" data-tx="${Math.max(8,Math.min(width-180,point.x+12)).toFixed(2)}" data-ty="${Math.max(18,point.y-12).toFixed(2)}"></circle><circle class="chart-point-hit" cx="${point.x.toFixed(2)}" cy="${point.y.toFixed(2)}" r="12" data-title="${safeChartText(point.item.label)}" data-value="${safeChartText(formatter(point.item[field]))}" data-detail="${safeChartText(point.item.detail)}" data-tx="${Math.max(8,Math.min(width-180,point.x+12)).toFixed(2)}" data-ty="${Math.max(18,point.y-12).toFixed(2)}"></circle>`).join('')};};
  const paceLine=buildLine('pace',paceColor,paceY,(value)=>formatPace(value));
  const cadenceLine=buildLine('cadence',cadenceColor,cadenceY,(value)=>`${formatNumber(value,0)} spm`);
  const hrLine=buildLine('hr',hrColor,hrY,(value)=>formatHeartRate(value));
  const xTicks=buildIndexedTicks(merged.length,tickCount);
  const paceTicks=buildChartTicks(paceDomain.min,paceDomain.max,5);
  const cadenceTicks=buildChartTicks(cadenceDomain.min,cadenceDomain.max,5);
  const hrTicks=buildChartTicks(hrDomain.min,hrDomain.max,5);
  container.innerHTML=`<div class="chart-shell curve-wrap"><svg class="chart-svg" viewBox="0 0 ${width} ${height}" preserveAspectRatio="none" role="img" aria-label="${safeChartText('Allure, cadence et fréquence cardiaque sur un même graphe')}"><line class="chart-axis-line" x1="${padLeft}" y1="${height-padBottom}" x2="${width-padRight}" y2="${height-padBottom}"></line><line class="chart-axis-line" x1="${padLeft}" y1="${padTop}" x2="${padLeft}" y2="${height-padBottom}"></line><line class="chart-axis-line" x1="${width-padRight}" y1="${padTop}" x2="${width-padRight}" y2="${height-padBottom}"></line><line class="chart-axis-line" x1="${width-padRight+34}" y1="${padTop}" x2="${width-padRight+34}" y2="${height-padBottom}"></line>${paceTicks.map((tick)=>{const y=paceY(tick);return `<g><line class="chart-grid-line" x1="${padLeft}" y1="${y.toFixed(2)}" x2="${width-padRight}" y2="${y.toFixed(2)}"></line><text class="chart-axis-tick" x="${padLeft-8}" y="${(y+4).toFixed(2)}" text-anchor="end">${safeChartText(formatPace(tick))}</text></g>`}).join('')}${cadenceTicks.map((tick)=>{const y=cadenceY(tick);return `<text class="chart-axis-tick" x="${width-padRight+6}" y="${(y+4).toFixed(2)}" text-anchor="start">${safeChartText(`${formatNumber(tick,0)} spm`)}</text>`}).join('')}${hrTicks.map((tick)=>{const y=hrY(tick);return `<text class="chart-axis-tick" x="${width-padRight+40}" y="${(y+4).toFixed(2)}" text-anchor="start">${safeChartText(`${formatNumber(tick,0)} bpm`)}</text>`}).join('')}${(options.hrReferenceBands||[]).filter((band)=>Number.isFinite(Number(band.low))&&Number.isFinite(Number(band.high))).map((band)=>{const top=hrY(Math.max(Number(band.low),Number(band.high)));const bottom=hrY(Math.min(Number(band.low),Number(band.high)));return `<rect x="${padLeft}" y="${Math.min(top,bottom).toFixed(2)}" width="${innerWidth}" height="${Math.abs(bottom-top).toFixed(2)}" fill="${band.color||'rgba(255,255,255,0.08)'}" opacity="0.2"></rect>`}).join('')}${(options.cadenceReferenceLines||[]).filter((line)=>Number.isFinite(Number(line.value))).map((line)=>{const y=cadenceY(line.value);return `<g><line class="chart-grid-line" x1="${padLeft}" y1="${y.toFixed(2)}" x2="${width-padRight}" y2="${y.toFixed(2)}" stroke-dasharray="4 4"></line><text class="chart-axis-tick" x="${width-padRight}" y="${(y-4).toFixed(2)}" text-anchor="end">${safeChartText(line.label||`${formatNumber(line.value,0)} spm`)}</text></g>`}).join('')}${xTicks.map((index)=>{const x=merged.length>1?padLeft+index*step:padLeft+innerWidth/2;return `<text class="chart-axis-tick" x="${x.toFixed(2)}" y="${height-8}" text-anchor="middle">${safeChartText(merged[index]?.label||'')}</text>`}).join('')}${paceLine.path}${cadenceLine.path}${hrLine.path}${paceLine.hits}${cadenceLine.hits}${hrLine.hits}<text class="chart-axis-title" x="${padLeft}" y="${16}">Allure</text><text class="chart-axis-title" x="${width-padRight}" y="${16}" text-anchor="end">Cadence</text><text class="chart-axis-title" x="${width-padRight+68}" y="${16}" text-anchor="end">FC</text></svg><div class="curve-legend"><span><i style="background:${paceColor}"></i>Allure</span><span><i style="background:${cadenceColor}"></i>Cadence</span><span><i style="background:${hrColor}"></i>FC</span></div><div class="chart-tooltip" aria-hidden="true"></div></div>`;
  const tooltip=chartTooltipLabel(container);
  attachChartTooltipHandlers(container,tooltip,Array.from(container.querySelectorAll('.chart-point-hit, .chart-point-dot')));
}function buildTrendModalCard(key){const payload=currentPayload();const cards=dashboardCardSpecs(payload);const directMap={hrv:'hrv',sleep:'sleep','resting-hr':'resting-hr',cadence:'cadence',load:'charge','load-ratio':'charge-ratio','running-volume':'weekly-volume','bike-volume':'bike-volume','pace-hr':'pace-hr','session-types':'session-types'};if(directMap[key])return cards.find((item)=>item.key===directMap[key])||null;const metrics=currentMetrics();const trend=currentAnalysis().trend||{};const windowed=buildTrendWindowSeries(trend,state.dashboardWindowDays);if(key==='zones-all'||key==='zones-running'){const zonesAll=trend.heart_rate_zone_share||metrics.heart_rate_zone_share||{distribution:[]};const zonesRunning=trend.heart_rate_zone_share_running||metrics.heart_rate_zone_share_running||{distribution:[]};return{key:'zones-all',title:'Zones FC',value:`${formatNumber((Math.max(zonesAll.total_seconds||0,zonesRunning.total_seconds||0))/3600,1)} h`,subtitle:`Vue unique avec switch · ${dashboardWindowDetail(state.dashboardWindowDays)}`,detail:'Un seul encart est conservé. Le switch compare toutes les activités et le running sans dupliquer le graphe.',calculation:'Répartition du temps en zones à partir de la durée d’activité et de la FC moyenne de chaque activité sur la fenêtre active.',provenance:'Activités Garmin normalisées + seuils de zones cardio issus du profil utilisateur.',reading:'Utiliser la vue Running pour l’analyse coaching spécifique, et Toutes activités pour voir la charge cardio globale sur la période choisie.',referenceNotes:['Les zones sont affichées avec leurs bornes BPM quand elles sont disponibles.'],chartType:'zones',zonesAll,zonesRunning,note:'Switch entre toutes les activités et le running uniquement, filtré sur la période active.'}}if(key==='pace-cadence-hr'){const cadenceTarget=metrics.cadence_target_spm||170;const cadenceRefLow=metrics.cadence_reference_low||cadenceTarget-5;const cadenceRefHigh=metrics.cadence_reference_high||cadenceTarget+5;const runningPaceSeries=Array.isArray(windowed.runningPaceSeries)?windowed.runningPaceSeries:[];const cadenceSeries=Array.isArray(windowed.cadenceSeries)?windowed.cadenceSeries:[];const runningHrSeries=Array.isArray(windowed.runningHrSeries)?windowed.runningHrSeries:[];const lastPace=runningPaceSeries.at(-1);const lastCadence=cadenceSeries.at(-1);const lastHr=runningHrSeries.at(-1);return{key,title:'Pace / cadence / FC',value:`${lastPace?formatPace(lastPace.pace_min_per_km):'-'} · ${lastCadence?`${formatNumber(lastCadence.cadence_spm,0)} spm`:'-'} · ${lastHr?formatHeartRate(lastHr.heart_rate):'-'}`,subtitle:`Vue combinée ${dashboardWindowDetail(state.dashboardWindowDays)} · 3 axes Y visibles`,detail:'Les trois séries sont superposées dans un même graphe avec trois axes Y visibles. Le hover donne les valeurs exactes au besoin.',calculation:`Séries journalières running alignées sur ${dashboardWindowDetail(state.dashboardWindowDays).toLowerCase()}, avec trois axes Y distincts.`,provenance:'Activités running Garmin normalisées, cadence issue des activités, FC issue des résumés running.',reading:'Chercher si la cadence monte ou se stabilise à allure comparable sans explosion de FC.',referenceNotes:[`Cadence cible : ${cadenceTarget} spm, bande ${cadenceRefLow}-${cadenceRefHigh} spm.`],signals:[{label:'Dernière journée alignée',value:lastPace?.metric_date?formatDateLabel(lastPace.metric_date):'-'}],chartType:'triptych',paceSeries:runningPaceSeries,cadenceSeries,hrSeries:runningHrSeries,xTickFormatter:(item)=>item.label||'',cadenceReferenceLines:[{value:cadenceRefLow,label:`bande basse ${formatNumber(cadenceRefLow,0)}`,color:'#8ab4ff'},{value:cadenceTarget,label:`cible ${cadenceTarget}`,color:'#8ae6c0'},{value:cadenceRefHigh,label:`bande haute ${formatNumber(cadenceRefHigh,0)}`,color:'#8ab4ff'}].filter((item)=>Number.isFinite(Number(item.value))),hrReferenceBands:[{low:metrics.resting_hr_reference_low,high:metrics.resting_hr_reference_high,color:'#8ab4ff',label:'FC repos'}].filter((band)=>Number.isFinite(Number(band.low))&&Number.isFinite(Number(band.high)))}}return null}
...
function dashboardCardSpecs(payload){
  ...
  const loadSeries=Array.isArray(windowed.loadSeries)?windowed.loadSeries:[];
  ...
  const sleepSeriesRaw=Array.isArray(windowed.sleepSeriesRaw)?windowed.sleepSeriesRaw:sleepSeries;
  const sleepSeriesSmoothed=Array.isArray(windowed.sleepSeriesSmoothed)?windowed.sleepSeriesSmoothed:sleepSeries;
  ...
  const hrvSeriesRaw=Array.isArray(windowed.hrvSeriesRaw)?windowed.hrvSeriesRaw:hrvSeries;
  const hrvSeriesSmoothed=Array.isArray(windowed.hrvSeriesSmoothed)?windowed.hrvSeriesSmoothed:hrvSeries;
  const cadenceSeries=Array.isArray(windowed.cadenceSeries)?windowed.cadenceSeries:[];
  const sessionTypeDistribution=Array.isArray(windowed.sessionTypeDistribution)?windowed.sessionTypeDistribution:[];
  const sessionTypeRows=Array.isArray(windowed.sessionTypeRows)?windowed.sessionTypeRows:[];
  const cadenceDiagnostics=windowed.cadenceDiagnostics||metrics.cadence_diagnostics||{activities:[],summary:{}};
  const cadenceSummary=cadenceDiagnostics.summary||{};
  ...
  const volumeSeries=runningVolumeSeries.map((row)=>({value:row.distance_km||0,label:row.label||formatShortDateLabel(row.metric_date),detail:row.detail||formatDateLabel(row.metric_date)}));
  const bikeSeries=bikeVolumeSeries.map((row)=>({value:row.distance_km||0,label:row.label||formatShortDateLabel(row.metric_date),detail:row.detail||formatDateLabel(row.metric_date)}));
  const loadChartSeries=loadSeries.map((row)=>({value:row.activity_load||row.value||0,label:row.label||formatShortDateLabel(row.metric_date),detail:row.detail||formatDateLabel(row.metric_date)})).filter((row)=>Number.isFinite(Number(row.value)));
  const loadRatioChartSeries=loadRatioSeries.map((row)=>({value:row.load_ratio_7_28,label:row.label||formatShortDateLabel(row.metric_date),detail:row.detail||formatDateLabel(row.metric_date)})).filter((row)=>Number.isFinite(Number(row.value)));
  const sleepChartSeries=sleepSeries.map((row)=>({value:row.sleep_hours,label:row.label||formatShortDateLabel(row.metric_date),detail:row.detail||formatDateLabel(row.metric_date)})).filter((row)=>Number.isFinite(Number(row.value)));
  const restingHrChartSeries=restingHrSeries.map((row)=>({value:row.resting_hr,label:row.label||formatShortDateLabel(row.metric_date),detail:row.detail||formatDateLabel(row.metric_date)})).filter((row)=>Number.isFinite(Number(row.value)));
  const hrvChartSeries=hrvSeries.map((row)=>({value:row.hrv_ms,label:row.label||formatShortDateLabel(row.metric_date),detail:row.detail||formatDateLabel(row.metric_date)})).filter((row)=>Number.isFinite(Number(row.value)));
  ...
  const sleepLast=sleepChartSeries.at(-1);
  const restingHrLast=restingHrChartSeries.at(-1);
  const hrvLast=hrvChartSeries.at(-1);
  const cadenceLast=cadenceSeries.at(-1);
  const ratioRefLow=metrics.load_ratio_reference_low ?? 0.9;
  const ratioRefHigh=metrics.load_ratio_reference_high ?? 1.2;
  const ratioRef=formatBand(ratioRefLow,ratioRefHigh,(value)=>formatNumber(value,2));
  ...
  const cadenceBounds=cadenceSummary.plot_bounds?.min!==null&&cadenceSummary.plot_bounds?.min!==undefined&&cadenceSummary.plot_bounds?.max!==null&&cadenceSummary.plot_bounds?.max!==undefined?`${formatNumber(cadenceSummary.plot_bounds.min,1)}-${formatNumber(cadenceSummary.plot_bounds.max,1)} spm`:'non calculées';
  const sessionTypesTotal=sessionTypeDistribution.reduce((sum,row)=>sum+Number(row.count||row.value||0),0);
  return [
    {key:'weekly-volume',title:'Volume running',value:formatKilometers(metrics.weekly_volume_km ?? metrics.total_distance_km_7d),subtitle:`Histogramme ${dashboardWindowDetail(state.dashboardWindowDays)} / ${formatNumber(metrics.weekly_running_days ?? metrics.recent_running_days ?? 0)} sorties`,detail:'Volume running uniquement. Le vélo reste dans un graphe séparé.',calculation:`Somme des kilomètres running visibles sur ${dashboardWindowDetail(state.dashboardWindowDays).toLowerCase()}. Sur 1 an, l’agrégation passe en semaines ISO.`,provenance:'Activités running Garmin normalisées dans le workspace.',reading:'Lire la régularité du volume et les ruptures de charge. Le format histogramme est plus honnête qu’une courbe reliée par des points pour des totaux journaliers ou hebdomadaires.',referenceNotes:['Le running reste séparé du vélo.','À 1 an, les barres sont hebdomadaires pour éviter un bruit journalier illisible.'],signals:[{label:'Dernier jour local',value:formatDateLabel(payload.latest_day || importStatus.latest_activity_day)},{label:'Dernier import',value:latestRun?formatDateTime(latestRun.finished_at):'-'}],chartType:'bars',series:volumeSeries,tickFormatter:formatAxisKilometers,yLabel:'km',tone:'primary'},
    {key:'bike-volume',title:'Volume vélo',value:formatKilometers(metrics.weekly_bike_volume_km),subtitle:`Histogramme ${dashboardWindowDetail(state.dashboardWindowDays)} / ${formatNumber(metrics.weekly_bike_days ?? metrics.recent_bike_days ?? 0)} sorties`,detail:'Le vélo reste isolé du running pour garder une lecture coaching propre.',calculation:`Somme des kilomètres vélo visibles sur ${dashboardWindowDetail(state.dashboardWindowDays).toLowerCase()}. Sur 1 an, l’agrégation passe en semaines ISO.`,provenance:'Activités vélo Garmin normalisées dans le workspace.',reading:'Lire la charge croisée sans contaminer le volume running.',referenceNotes:['Le vélo n’est pas regroupé avec le running.','Le graphe est volontairement séparé pour garder deux lectures distinctes.'],signals:[{label:'7 jours',value:formatKilometers(metrics.weekly_bike_volume_km)},{label:'Dernier import',value:latestRun?formatDateTime(latestRun.finished_at):'-'}],chartType:'bars',series:bikeSeries,tickFormatter:formatAxisKilometers,yLabel:'km'},
    {key:'charge',title:'Charge quotidienne',value:metrics.load_7d===null||metrics.load_7d===undefined?'-':formatNumber(metrics.load_7d,0),subtitle:`Brute ${dashboardWindowDetail(state.dashboardWindowDays)}`,detail:'La charge quotidienne montre la somme journalière du training load running, sans moyenne glissante par défaut.',calculation:'Somme journalière du `training_load` running. Chaque barre ou point correspond à la charge brute du jour visible dans la fenêtre active.',provenance:'Activités running Garmin normalisées, champ `training_load` agrégé par jour dans `derived_daily_metrics`.',reading:'Lire la densité de charge jour par jour. Les jours à zéro signalent soit du repos, soit l’absence de running sur cette journée.',referenceNotes:[`Bande de lecture actuelle ${formatBand(metrics.load_reference_low,metrics.load_reference_high,(value)=>formatNumber(value,0))}.`],signals:[{label:'7 jours',value:formatNumber(metrics.load_7d,0)},{label:'28 jours',value:formatNumber(metrics.load_28d,0)}],chartType:'single',series:loadChartSeries,tickFormatter:(value)=>formatNumber(value,0),stroke:'#7fd6ff',fill:'rgba(127, 214, 255, 0.18)',yLabel:'u',drawArea:false,referenceLines:[{value:metrics.load_reference_low,label:`bande basse ${formatNumber(metrics.load_reference_low,0)}`,color:'#8ab4ff'},{value:metrics.load_reference_high,label:`bande haute ${formatNumber(metrics.load_reference_high,0)}`,color:'#8ab4ff'}].filter((item)=>Number.isFinite(Number(item.value)))},
    {key:'charge-ratio',title:'Charge relative',value:metrics.load_ratio_7_28===null||metrics.load_ratio_7_28===undefined?'-':formatNumber(metrics.load_ratio_7_28,2),subtitle:`Réf ${ratioRef} · cible ${formatNumber(metrics.load_ratio_target||1,2)}`,detail:'Le ratio compare la charge 7 jours récente à une charge chronique ramenée à un équivalent 7 jours.',calculation:'Calcul = `load_7d / (load_28d / 4)`. Si les 28 derniers jours sont stables, la valeur tourne autour de `1.00`.',provenance:'Table `derived_daily_metrics`, calculée localement à partir des activités Garmin normalisées et des charges dérivées.',reading:'Autour de `1.00`, la dynamique est stable. Sous `0.80`, on est nettement en retrait. Au-dessus de `1.20`, la charge récente dépasse la base chronique. Au-dessus de `1.30`, on surveille franchement la surcharge.',referenceNotes:[`Bande de lecture retenue ${ratioRef}.`,`La ligne 1.00 correspond à l’équivalent de la charge chronique ramenée à 7 jours.`],signals:[{label:'Fatigue',value:metrics.fatigue_flag?'Oui':'Non'},{label:'Overreaching',value:metrics.overreaching_flag?'Oui':'Non'}],chartType:'single',series:loadRatioChartSeries,tickFormatter:(value)=>formatNumber(value,2),stroke:'#ffbe55',fill:'rgba(255, 190, 85, 0.22)',referenceLines:[{value:ratioRefLow,label:`bande basse ${formatNumber(ratioRefLow,2)}`,color:'#8ab4ff'},{value:metrics.load_ratio_target||1.0,label:'cible 1.00',color:'#8ae6c0'},{value:ratioRefHigh,label:`bande haute ${formatNumber(ratioRefHigh,2)}`,color:'#8ab4ff'}].filter((item)=>Number.isFinite(Number(item.value))),referenceBands:[{low:ratioRefLow,high:ratioRefHigh,color:'rgba(255, 190, 85, 0.14)'}].filter((band)=>Number.isFinite(Number(band.low))&&Number.isFinite(Number(band.high)))},
    {key:'sleep',title:'Sommeil',value:sleepLast?`${formatNumber(sleepLast.value,1)} h`:'-',subtitle:`${state.chartModes.sleep==='smooth'?'Moyenne glissante 7 jours':'Valeur brute journalière'} / mode changeable`,detail:'Le sommeil arrive brut par défaut. Un bouton permet d’afficher la moyenne glissante 7 jours à la demande sans la mémoriser.',calculation:state.chartModes.sleep==='smooth'?'Moyenne glissante 7 jours des heures de sommeil journalières.':'Agrégation journalière brute depuis `wellness_daily` avec conversion des secondes en heures.',provenance:'Mesures wellness Garmin normalisées, lues depuis `wellness_daily` puis dérivées localement pour la version lissée.',reading:'Commencer par le brut pour voir la variabilité réelle. Passer en lissé 7 jours quand on veut lire une tendance de récupération.',referenceNotes:['Valeur brute journalière par défaut.','Le lissage 7 jours reste optionnel et non mémorisé.'],signals:[{label:'Dernière nuit',value:sleepLast?.detail||'-'},{label:'Mode',value:state.chartModes.sleep==='smooth'?'Lissé 7 jours':'Brut'}],chartType:'single',series:sleepChartSeries,tickFormatter:(value)=>`${formatNumber(value,1)} h`,stroke:'#8ae6c0',fill:'rgba(138, 230, 192, 0.2)',yLabel:'h',drawArea:false,smoothingModes:['raw','smooth'],referenceLines:[{value:metrics.sleep_reference_low,label:`bande basse ${formatNumber(metrics.sleep_reference_low,1)} h`,color:'#8ab4ff'},{value:metrics.sleep_reference_high,label:`bande haute ${formatNumber(metrics.sleep_reference_high,1)} h`,color:'#8ab4ff'}].filter((item)=>Number.isFinite(Number(item.value))),referenceBands:[{low:metrics.sleep_reference_low,high:metrics.sleep_reference_high,color:'rgba(138, 230, 192, 0.12)'}].filter((band)=>Number.isFinite(Number(band.low))&&Number.isFinite(Number(band.high))),rawSeries:sleepSeriesRaw,smoothedSeries:sleepSeriesSmoothed},
    {key:'resting-hr',title:'FC repos',value:restingHrLast?formatHeartRate(restingHrLast.value):'-',subtitle:'Valeur brute journalière / sans lissage par défaut',detail:'La FC repos affiche la mesure journalière brute.',calculation:'Agrégation journalière brute depuis `wellness_daily`.',provenance:'Mesures wellness Garmin normalisées, lues depuis `wellness_daily`.',reading:'Une dérive vers le haut se lit avec la HRV et le sommeil, sans masquer les variations jour par jour.',referenceNotes:['Valeur brute journalière, sans moyenne glissante par défaut.','La bande de référence reste une aide de lecture, pas une cible à suivre seule.'],signals:[{label:'Dernière mesure',value:restingHrLast?.detail||'-'},{label:'Points visibles',value:String(restingHrChartSeries.length)}],chartType:'single',series:restingHrChartSeries,tickFormatter:(value)=>`${formatNumber(value,0)} bpm`,stroke:'#8ab4ff',fill:'rgba(138, 180, 255, 0.2)',yLabel:'bpm',drawArea:false,referenceLines:[{value:metrics.resting_hr_reference_low,label:`bande basse ${formatNumber(metrics.resting_hr_reference_low,0)} bpm`,color:'#8ab4ff'},{value:metrics.resting_hr_reference_high,label:`bande haute ${formatNumber(metrics.resting_hr_reference_high,0)} bpm`,color:'#8ab4ff'}].filter((item)=>Number.isFinite(Number(item.value))),referenceBands:[{low:metrics.resting_hr_reference_low,high:metrics.resting_hr_reference_high,color:'rgba(138, 180, 255, 0.12)'}].filter((band)=>Number.isFinite(Number(band.low))&&Number.isFinite(Number(band.high)))},
    {key:'hrv',title:'HRV',value:hrvLast?`${formatNumber(hrvLast.value,1)} ms`:'-',subtitle:`${state.chartModes.hrv==='smooth'?'Moyenne glissante 7 jours':'Valeur brute journalière'} / mode changeable`,detail:'La HRV arrive brute par défaut. Un bouton permet d’afficher la moyenne glissante 7 jours à la demande.',calculation:state.chartModes.hrv==='smooth'?'Moyenne glissante 7 jours de la HRV journalière.':'Agrégation journalière brute depuis `wellness_daily`.',provenance:'Mesures wellness Garmin normalisées, lues depuis `wellness_daily` puis dérivées localement pour la version lissée.',reading:'Observer d’abord la variabilité réelle. Passer en lissé 7 jours pour lire une tendance de fond sans masquer complètement le signal.',referenceNotes:['Valeur brute journalière par défaut.','La lecture de HRV se fait toujours avec le contexte de charge, de sommeil et de FC repos.'],signals:[{label:'Dernière mesure',value:hrvLast?.detail||'-'},{label:'Mode',value:state.chartModes.hrv==='smooth'?'Lissé 7 jours':'Brut'}],chartType:'single',series:hrvChartSeries,tickFormatter:(value)=>`${formatNumber(value,1)} ms`,stroke:'#a38bff',fill:'rgba(163, 139, 255, 0.16)',yLabel:'ms',drawArea:false,smoothingModes:['raw','smooth'],referenceBands:[{low:metrics.hrv_reference_low,high:metrics.hrv_reference_high,color:'rgba(163, 139, 255, 0.12)'}].filter((band)=>Number.isFinite(Number(band.low))&&Number.isFinite(Number(band.high))),rawSeries:hrvSeriesRaw,smoothedSeries:hrvSeriesSmoothed},
    {key:'pace-hr',title:'Pace / FC',value:paceCurveLast?`${formatPace(paceCurveLast.pace_min_per_km)} / ${formatHeartRate(paceCurveLast.heart_rate)}`:'-',subtitle:`Courbe monotone / ${paceCurve.length} points utiles`,detail:'La courbe conserve les points utiles des dernières sorties, puis les lisse pour rester monotone.',calculation:'Agrégation des points running utiles, exclusion des fractions trop courtes, puis lissage monotone de la relation allure / FC.',provenance:'Activités running locales Garmin, enrichies cadence et FC.',reading:'Si la courbe se décale vers le bas pour une allure donnée, l’efficacité aérobie progresse en général.',referenceNotes:[benchmark?.event?`Repère actuel : ${benchmarkText}`:'Pas de benchmark retenu sur cette fenêtre.','Les points courts, instables ou sans FC exploitable sont retirés de la courbe.'],signals:[{label:'Premier point',value:paceCurveFirst?`${formatPace(paceCurveFirst.pace_min_per_km)} / ${formatHeartRate(paceCurveFirst.heart_rate)}`:'-'},{label:'Benchmark retenu',value:benchmarkSignal}],chartType:'curve',curve:paceCurve,maxHrEstimate:metrics.max_hr_estimate,diagnostics:paceCurveDebug},
    {key:'cadence',title:'Cadence',value:cadenceLast?`${formatNumber(cadenceLast.cadence_spm||cadenceLast.value,0)} spm`:'-',subtitle:`Diagnostic axe ${cadenceBounds}`,detail:'La cadence doit rester en steps per minute. Le bloc de diagnostic sert à vérifier la source, l’unité détectée et les bornes du graphe.',calculation:'Cadence running convertie en steps per minute à partir des champs Garmin doubles quand ils existent, sinon conversion depuis une cadence jambe.',provenance:'Activités running Garmin normalisées, plus diagnostic exposé par `cadence_diagnostics`.',reading:'Si les valeurs sources tournent autour de 155 spm mais que l’axe graphique reste vers 44-70, il y a un bug de domaine ou d’unité. Le diagnostic ci-dessous sert à le vérifier explicitement.',referenceNotes:[`Cible ${cadenceTarget} spm, bande ${cadenceRefLow}-${cadenceRefHigh} spm.`,`Bornes d’axe détectées actuellement : ${cadenceBounds}.`],signals:[{label:'Activités diagnostiquées',value:String(cadenceSummary.activity_count ?? cadenceDiagnostics.activities?.length ?? 0)},{label:'Points tracés',value:String(cadenceSummary.plotted_point_count ?? cadenceSeries.length)}],chartType:'single',series:cadenceSeries,tone:'primary',yLabel:'spm',tickFormatter:(value)=>`${formatNumber(value,0)} spm`,drawArea:false,referenceLines:[{value:cadenceRefLow,label:`bande basse ${formatNumber(cadenceRefLow,0)} spm`,color:'#8ab4ff'},{value:cadenceTarget,label:`cible ${cadenceTarget} spm`,color:'#8ae6c0'},{value:cadenceRefHigh,label:`bande haute ${formatNumber(cadenceRefHigh,0)} spm`,color:'#8ab4ff'}].filter((item)=>Number.isFinite(Number(item.value)))},
    {key:'session-types',title:'Types de séance',value:sessionTypesTotal?`${sessionTypesTotal} séances`:'-',subtitle:`Répartition ${dashboardWindowDetail(state.dashboardWindowDays)}`,detail:'Les séances running sont classées en jogg facile, qualité ou sortie longue selon une heuristique locale documentée.',calculation:'Classification running locale : sortie longue d’abord par durée/distance haute, qualité ensuite par signaux d’intensité ou de structure, jogg facile sinon.',provenance:'Activités running Garmin normalisées, distance, durée, FC moyenne, training load et présence de splits/workout dans le payload brut.',reading:'Lire le mélange récent des séances. Un bloc très concentré sur la qualité ou l’absence prolongée de sortie longue change l’interprétation du cycle.',referenceNotes:['Le typage est heuristique et documenté dans le document technique dédié.','La période sélectionnée change directement la distribution affichée.'],signals:[{label:'Jogg facile',value:String(sessionTypeDistribution.find((item)=>item.session_type==='easy_jog')?.count||0)},{label:'Qualité',value:String(sessionTypeDistribution.find((item)=>item.session_type==='quality')?.count||0)},{label:'Sortie longue',value:String(sessionTypeDistribution.find((item)=>item.session_type==='long_run')?.count||0)}],chartType:'bars',series:sessionTypeDistribution,tickFormatter:(value)=>formatNumber(value,0),yLabel:'séances'}
  ];
}
function renderDashboardModal(card){
  ...
  try{
    ...
    if(Array.isArray(card.smoothingModes)&&card.smoothingModes.length){
      const controls=document.createElement('div');
      controls.className='window-controls';
      const currentMode=state.chartModes?.[card.key]||'raw';
      [{mode:'raw',label:'Brut'},{mode:'smooth',label:'Lissé 7 jours'}].filter((entry)=>card.smoothingModes.includes(entry.mode)).forEach((entry)=>{const button=document.createElement('button');button.type='button';button.className=`dashboard-window-button${currentMode===entry.mode?' active':''}`;button.textContent=entry.label;button.addEventListener('click',()=>{state.chartModes[card.key]=entry.mode;const refreshed=resolveDashboardModalCard(card.key);if(refreshed)renderDashboardModal(refreshed);renderDashboardSection()});controls.appendChild(button)});
      dom.dashboardModalSignals.appendChild(controls);
    }
    ...
    }else if(card.chartType==='bars'){
      renderBarChart(dom.dashboardModalChart,card.series||[],{id:`modal-${card.key}`,stroke:card.stroke||'#8ae6c0',width:1200,height:380,tickFormatter:card.tickFormatter,xTickCount:chartWindowTickCount(state.dashboardWindowDays),title:card.title,yLabel:card.yLabel||'km',emptyTitle:readiness.label||'Données indisponibles',emptyReason,emptyNotes,emptyAdvice:'Lance un recalcul si les données existent mais ne s’affichent pas encore.'});
...
function renderDashboardSection(){
  ...
  const sessionTypeDistribution=Array.isArray(windowed.sessionTypeDistribution)?windowed.sessionTypeDistribution:[];
  const zonesAll=trend.heart_rate_zone_share||metrics.heart_rate_zone_share||{distribution:[]};
  const zonesRunning=trend.heart_rate_zone_share_running||metrics.heart_rate_zone_share_running||{distribution:[]};
  const cadenceDiagnostics=windowed.cadenceDiagnostics||metrics.cadence_diagnostics||{activities:[],summary:{}};
  const cadenceSummary=cadenceDiagnostics.summary||{};
  const paceCurveDebug=windowed.paceCurveDebug||metrics.pace_hr_curve_debug||{};
  const cadenceTarget=metrics.cadence_target_spm||170;
  ...
  const windowText=dashboardWindowDetail(state.dashboardWindowDays);
  const lastSleep=sleepSeries.at(-1);
  const lastResting=restingHrSeries.at(-1);
  const lastHrv=hrvSeries.at(-1);
  const lastCadence=cadenceSeries.at(-1);
  const cadenceBounds=cadenceSummary.plot_bounds?.min!==null&&cadenceSummary.plot_bounds?.min!==undefined&&cadenceSummary.plot_bounds?.max!==null&&cadenceSummary.plot_bounds?.max!==undefined?`${formatNumber(cadenceSummary.plot_bounds.min,1)}-${formatNumber(cadenceSummary.plot_bounds.max,1)} spm`:'non calculées';
  ['volume-trend-window','bike-volume-trend-window','load-ratio-trend-window','load-trend-window','sleep-trend-window','resting-hr-trend-window','hrv-trend-window','pace-hr-trend-window','cadence-trend-window','pace-cadence-hr-trend-window','session-types-trend-window'].forEach((id)=>{const node=document.getElementById(id);if(node)node.textContent=windowText});
  ...
  dom.dashboardImportDetail.textContent=metrics.weekly_running_days!==null&&metrics.weekly_running_days!==undefined?`${formatNumber(metrics.weekly_running_days,0)} sorties sur 7 j / ${runningSeries.length} points visibles`:'Volume running non disponible';
  ...
  if(dom.dashboardHrvState) dom.dashboardHrvState.textContent=lastHrv?`${formatNumber(lastHrv.hrv_ms||lastHrv.value,1)} ms`:'-';
  if(dom.dashboardHrvDetail) dom.dashboardHrvDetail.textContent=lastHrv?.metric_date?`Brut journalier / ${formatDateLabel(lastHrv.metric_date)}`:'HRV brute non disponible';
  ...
  dom.analysisCard.textContent=readiness.label||'Analyse';
  dom.analysisSubtitle.textContent=(readiness.details||[]).join(' / ')||readiness.reason||'Aucun diagnostic disponible';
  dom.volumeTrendSummary.textContent=runningSeries.length?`${formatKilometers(runningSeries.reduce((sum,row)=>sum+(row.distance_km||0),0))} / ${runningSeries.length} barres`:'Pas encore de volume running';
  renderBarChart(dom.volumeTrend,runningSeries,{...buildEmptyChartOptions(readiness.chartStates.volume),id:'volume',stroke:'#8ae6c0',height:128,width:360,title:'Volume running',yLabel:'km',tickFormatter:formatAxisKilometers,xTickCount:chartTicks});
  if(dom.bikeVolumeTrendSummary) dom.bikeVolumeTrendSummary.textContent=bikeSeries.length?`${formatKilometers(bikeSeries.reduce((sum,row)=>sum+(row.distance_km||0),0))} / ${bikeSeries.length} barres`:'Pas encore de volume vélo';
  if(dom.bikeVolumeTrend) renderBarChart(dom.bikeVolumeTrend,bikeSeries,{...buildEmptyChartOptions(readiness.chartStates.bike),id:'bike-volume',stroke:'#8ab4ff',height:128,width:360,title:'Volume vélo',yLabel:'km',tickFormatter:formatAxisKilometers,xTickCount:chartTicks});
  if(dom.loadTrendSummary) dom.loadTrendSummary.textContent=loadSeries.length?`${formatNumber(loadSeries.at(-1)?.activity_load||loadSeries.at(-1)?.value||0,0)} u / ${loadSeries.length} jours`:'Pas encore de charge quotidienne';
  if(dom.loadTrend) renderSparkline(dom.loadTrend,loadSeries,{...buildEmptyChartOptions(readiness.chartStates.load),id:'load',stroke:'#7fd6ff',fill:'rgba(127, 214, 255, 0.18)',height:128,width:360,title:'Charge quotidienne',yLabel:'u',tickFormatter:(value)=>formatNumber(value,0),xTickCount:chartTicks,drawArea:false});
  ...
  if(dom.loadRatioTrend) renderSparkline(dom.loadRatioTrend,loadRatioSeries,{...buildEmptyChartOptions(readiness.chartStates.loadRatio),id:'load-ratio',stroke:'#ffbe55',fill:'rgba(255, 190, 85, 0.22)',height:128,width:360,title:'Charge relative',yLabel:'ratio',tickFormatter:(value)=>formatNumber(value,2),referenceLines:[{value:metrics.load_ratio_reference_low||0.8,label:`bande basse ${formatNumber(metrics.load_ratio_reference_low||0.8,2)}`,color:'#8ab4ff'},{value:metrics.load_ratio_target||1,label:'cible 1.00',color:'#8ae6c0'},{value:metrics.load_ratio_reference_high||1.2,label:`bande haute ${formatNumber(metrics.load_ratio_reference_high||1.2,2)}`,color:'#8ab4ff'}],xTickCount:chartTicks});
  if(dom.sleepTrendSummary) dom.sleepTrendSummary.textContent=lastSleep?`${formatNumber(lastSleep.sleep_hours||lastSleep.value,1)} h / ${state.chartModes.sleep==='smooth'?'lissé 7 j':'brut'}`:'Pas encore de sommeil';
  ...
  if(dom.restingHrTrendSummary) dom.restingHrTrendSummary.textContent=lastResting?`${formatNumber(lastResting.resting_hr||lastResting.value,0)} bpm / brut`:'Pas encore de FC repos';
  ...
  if(dom.hrvTrendSummary) dom.hrvTrendSummary.textContent=lastHrv?`${formatNumber(lastHrv.hrv_ms||lastHrv.value,1)} ms / ${state.chartModes.hrv==='smooth'?'lissé 7 j':'brut'}`:'Pas encore de HRV';
  ...
  dom.cadenceTrendSummary.textContent=lastCadence?`${formatNumber(lastCadence.cadence_spm||lastCadence.value,0)} spm / axe ${cadenceBounds}`:'Pas encore de cadence';
  renderSparkline(dom.cadenceTrend,cadenceSeries,{...buildEmptyChartOptions(readiness.chartStates.cadence),id:'cadence',stroke:'#8ab4ff',fill:'rgba(138, 180, 255, 0.26)',height:128,width:360,title:'Cadence',yLabel:'spm',tickFormatter:(value)=>`${formatNumber(value,0)} spm`,referenceLines:[{value:cadenceRefLow,label:`bande basse ${formatNumber(cadenceRefLow,0)} spm`,color:'#8ab4ff'},{value:cadenceTarget,label:`cible ${formatNumber(cadenceTarget,0)} spm`,color:'#8ae6c0'},{value:cadenceRefHigh,label:`bande haute ${formatNumber(cadenceRefHigh,0)} spm`,color:'#8ab4ff'}],xTickCount:chartTicks,drawArea:false});
  if(dom.paceCadenceHrTrendSummary) dom.paceCadenceHrTrendSummary.textContent=runningPaceSeries.length&&runningHrSeries.length&&cadenceSeries.length?`${formatPace(runningPaceSeries.at(-1)?.pace_min_per_km)} / ${formatNumber(cadenceSeries.at(-1)?.cadence_spm||metrics.cadence_7d||0,0)} spm / ${formatHeartRate(runningHrSeries.at(-1)?.heart_rate || metrics.running_hr_7d)}`:'Vue combinée en attente';
  if(dom.paceCadenceHrTrend) renderTriptychChart(dom.paceCadenceHrTrend,runningPaceSeries,cadenceSeries,runningHrSeries,{id:'pace-cadence-hr',xTickFormatter:(item)=>item.label||'',cadenceReferenceLines:[{value:cadenceRefLow,label:`bande basse ${formatNumber(cadenceRefLow,0)} spm`,color:'#8ab4ff'},{value:cadenceTarget,label:`cible ${cadenceTarget}`,color:'#8ae6c0'},{value:cadenceRefHigh,label:`bande haute ${formatNumber(cadenceRefHigh,0)} spm`,color:'#8ab4ff'}].filter((item)=>Number.isFinite(Number(item.value))),hrReferenceBands:[{low:metrics.resting_hr_reference_low,high:metrics.resting_hr_reference_high,color:'rgba(138, 180, 255, 0.12)',label:'FC repos'}].filter((band)=>Number.isFinite(Number(band.low))&&Number.isFinite(Number(band.high))),xTickCount:chartTicks});
  if(dom.sessionTypesTrendSummary) dom.sessionTypesTrendSummary.textContent=sessionTypeDistribution.length?sessionTypeDistribution.map((item)=>`${item.label} ${item.count||item.value||0}`).join(' · '):'Pas encore de séances running classées';
  if(dom.sessionTypesTrend) renderBarChart(dom.sessionTypesTrend,sessionTypeDistribution,{...buildEmptyChartOptions(readiness.chartStates.sessionTypes),id:'session-types',stroke:'#ff8f6b',height:128,width:360,title:'Types de séance',yLabel:'séances',tickFormatter:(value)=>formatNumber(value,0),xTickCount:3});
  if(dom.zonesAllSummary) dom.zonesAllSummary.textContent=Math.max(zonesAll?.total_seconds||0,zonesRunning?.total_seconds||0)?`${formatNumber(Math.max(zonesAll?.total_seconds||0,zonesRunning?.total_seconds||0)/3600,1)} h / switch`:'-';
  if(dom.zonesAllChart) renderDonutComparison(dom.zonesAllChart,zonesAll,zonesRunning,{id:'zones',overallLabel:'Toutes activités',runningLabel:'Running',note:'Bascule entre toutes les activités et le running. Les bornes BPM des zones sont affichées dans la légende.'});
  dom.dashboardCards.innerHTML='';
  dashboardCardSpecs(payload).forEach((card)=>{const button=document.createElement('button');button.type='button';button.className=`metric-card${card.tone==='primary'?' is-primary':''}`;button.innerHTML=`<span class='card-label'>${safeChartText(card.title)}</span><strong>${safeChartText(card.value)}</strong><p>${safeChartText(card.subtitle||'')}</p>`;button.addEventListener('click',()=>{state.openTrendKey=card.key;addTerminalLog('info','dashboard',`Carte ouverte: ${card.title}`);renderDashboardModal(card)});dom.dashboardCards.appendChild(button)});
...
async function refreshDashboard({quiet=false,reason='manual',days=state.dashboardWindowDays}={}){persistSettings();const runner=async()=>{recordBoot('dashboard:refresh-start',`reason=${reason}`);const payload=await requestJson(`/api/status?data_dir=${encodeURIComponent(state.workspace)}&provider=${encodeURIComponent(state.provider)}&base_url=${encodeURIComponent(state.baseUrl)}&probe=1&days=${encodeURIComponent(normalizeDashboardWindowDays(days))}`,{},'status');if(payload.goal_profile)applyCoachProfile(payload.goal_profile,{preferExisting:true});if(payload.latest_plan?.available&&payload.latest_plan.plan&&typeof payload.latest_plan.plan==='object'){storeLatestPlanPayload(payload.latest_plan.plan)}else{clearLatestPlanPayload()}state.dashboardPayload=payload;renderAllSections();recordBoot('dashboard:refresh-ok',`reason=${reason}`);return payload};if(quiet)return runner();return withBusy('Lecture du workspace local...',runner)}
...
function renderSummary(payload){const analysis=payload.analysis||{};const lines=[];if(payload.coach_summary)lines.push(payload.coach_summary);if(analysis.training_phase)lines.push(`Phase: ${analysis.training_phase}`);if(analysis.benchmark?.event){const pace=analysis.benchmark.pace_min_per_km?` · ${formatPace(analysis.benchmark.pace_min_per_km)}`:'';lines.push(`Benchmark retenu: ${analysis.benchmark.event}${pace}`)}if(dom.coachSummary) dom.coachSummary.textContent=lines.join('\n\n')||'Aucun plan généré pour le moment.';if(dom.planList) dom.planList.innerHTML='';(payload.weekly_plan||[]).forEach((session)=>{const item=document.createElement('article');const sessionKind=classifyPlanSession(session);item.className=`plan-item${sessionKind==='rest'?' is-rest':sessionKind==='easy'?' is-easy':sessionKind==='quality'?' is-quality':''}`;item.innerHTML=`<strong>${safeChartText(session.day)} - ${safeChartText(session.session_title)}</strong><span>${safeChartText(`${session.duration_minutes} min | ${session.intensity}`)}</span><p>${safeChartText(session.objective||'')}</p><p>${safeChartText(session.notes||'')}</p>`;dom.planList?.appendChild(item)});const meta=[];if(payload.signals_used?.length)meta.push(`Signaux utilisés: ${payload.signals_used.join(', ')}`);if(analysis.training_phase)meta.push(`Phase détectée: ${analysis.training_phase}`);if(analysis.benchmark?.event){meta.push(`Repère récent: ${analysis.benchmark.event}${analysis.benchmark.pace_min_per_km?` · ${formatPace(analysis.benchmark.pace_min_per_km)}`:''}`)}renderCoachMeta(dom.planMeta,'Infos utilisées',meta)}
function renderQuestionResponse(payload){const lines=[];if(payload.coach_answer)lines.push(payload.coach_answer);if(payload.follow_up)lines.push(payload.follow_up);if(dom.questionResponse) dom.questionResponse.textContent=lines.join('\n\n')||'Réponse coaching indisponible.';const meta=[];if(payload.plan_context_note)meta.push(payload.plan_context_note);if(payload.signals_used?.length)meta.push(`Signaux utilisés: ${payload.signals_used.join(', ')}`);if(payload.question_text)meta.push(`Question envoyée: ${payload.question_text}`);renderCoachMeta(dom.questionMeta,'Contexte',meta)}
async function generatePlan(){persistSettings();const goalText=normalizeText((dom.goalInput?.value||'').trim());if(!goalText){if(dom.coachSummary) dom.coachSummary.textContent='Renseigne d’abord un objectif pour générer un planning.';renderCoachMeta(dom.planMeta,'Infos utilisées',[]);addTerminalLog('warn','coach','Plan impossible','Objectif manquant.');return}await flushCoachProfileSync({force:true,quiet:true});const profile=buildCoachProfilePayload();addTerminalLog('info','coach','Génération du planning',goalText);try{const payload=await withBusy('Le modèle prépare le planning...',async()=>requestJson('/api/coach/plan',{method:'POST',body:JSON.stringify({goal_text:goalText,data_dir:state.workspace,provider:state.provider,base_url:state.baseUrl||null,api_key:state.apiKey||null,profile})},'coach-plan'));if(payload.goal_profile)applyCoachProfile(payload.goal_profile,{preferExisting:false});storeLatestPlanPayload(payload);renderSummary(payload);state.dashboardPayload=payload.dashboard||state.dashboardPayload;renderAllSections();addTerminalLog('info','coach','Planning généré',payload.plan_path||'plan local enregistré')}catch(error){showRetryableError(error,generatePlan)}}
...
async function askCoachQuestion(){persistSettings();const goalText=normalizeText((dom.goalInput?.value||'').trim());const questionText=normalizeText((dom.questionInput?.value||'').trim());if(!goalText){if(dom.questionResponse) dom.questionResponse.textContent='Renseigne d’abord un objectif avant de poser une question ciblée.';renderCoachMeta(dom.questionMeta,'Contexte',[]);addTerminalLog('warn','coach','Question impossible','Objectif manquant.');return}if(!questionText){if(dom.questionResponse) dom.questionResponse.textContent='Écris d’abord une question sur le plan ou le ressenti récent.';renderCoachMeta(dom.questionMeta,'Contexte',[]);addTerminalLog('warn','coach','Question impossible','Question manquante.');return}await flushCoachProfileSync({force:true,quiet:true});const profile=buildCoachProfilePayload();addTerminalLog('info','coach','Question ciblée',questionText);try{const payload=await withBusy('Le coach analyse la question ciblée...',async()=>requestJson('/api/coach/question',{method:'POST',body:JSON.stringify({goal_text:goalText,question_text:questionText,data_dir:state.workspace,provider:state.provider,base_url:state.baseUrl||null,api_key:state.apiKey||null,profile})},'coach-question'));if(payload.goal_profile)applyCoachProfile(payload.goal_profile,{preferExisting:false});storeLatestQuestionPayload(payload);renderQuestionResponse(payload);state.dashboardPayload=payload.dashboard||state.dashboardPayload;renderAllSections();addTerminalLog('info','coach','Question traitée',payload.plan_context_available?payload.plan_context_path||'plan utilisé':'sans plan enregistré')}catch(error){showRetryableError(error,askCoachQuestion)}}
