...
const APP_VERSION='20260414-navfix25';
...
function renderSparkline(container,series,options={}){
  ...
  if(!items.length){container.innerHTML="<p class='status-line'>Pas encore de données exploitables.</p>";return}
...
}function buildTrendModalCard(key){const payload=currentPayload();const analysis=currentAnalysis();const metrics=currentMetrics();const trend=analysis.trend||{};const importStatus=currentImportStatus();const benchmark=analysis.benchmark||{};const paceCurve=Array.isArray(trend.pace_hr_curve)?trend.pace_hr_curve:[];const paceCurveDebug=metrics.pace_hr_curve_debug||{};const runningSeries=Array.isArray(trend.daily_volume)?trend.daily_volume:[];const bikeSeries=Array.isArray(trend.daily_bike_volume)?trend.daily_bike_volume:[];const loadRatioSeries=Array.isArray(trend.daily_load_ratio)?trend.daily_load_ratio:[];const sleepSeries=Array.isArray(trend.daily_sleep)?trend.daily_sleep:[];const restingHrSeries=Array.isArray(trend.daily_resting_hr)?trend.daily_resting_hr:[];const hrvSeries=Array.isArray(trend.daily_hrv)?trend.daily_hrv:[];const cadenceSeries=Array.isArray(trend.cadence_daily)?trend.cadence_daily:[];const runningPaceSeries=Array.isArray(trend.daily_running_pace)?trend.daily_running_pace:[];const runningHrSeries=Array.isArray(trend.daily_running_hr)?trend.daily_running_hr:[];const zonesAll=metrics.heart_rate_zone_share||trend.heart_rate_zone_share||{distribution:[]};const zonesRunning=metrics.heart_rate_zone_share_running||trend.heart_rate_zone_share_running||{distribution:[]};const cadenceTarget=metrics.cadence_target_spm||170;const cadenceRefLow=metrics.cadence_reference_low||cadenceTarget-5;const cadenceRefHigh=metrics.cadence_reference_high||cadenceTarget+5;const curveGap=paceCurveDebug.ready?`${paceCurveDebug.curve_points||paceCurve.length} points utiles`:'DonnÃ©es insuffisantes';const runLatest=runningSeries.at(-1)?.distance_km||metrics.weekly_volume_km||null;const bikeLatest=bikeSeries.at(-1)?.distance_km||metrics.weekly_bike_volume_km||null;const loadLatest=loadRatioSeries.at(-1)?.load_ratio_7_28;const sleepLatest=sleepSeries.at(-1)?.sleep_hours;const restingLatest=restingHrSeries.at(-1)?.resting_hr;const hrvLatest=hrvSeries.at(-1)?.hrv_ms;const cadenceLatest=cadenceSeries.at(-1)?.cadence_spm||metrics.cadence_7d;const paceLatest=runningPaceSeries.at(-1)?.pace_min_per_km||metrics.running_pace_7d;const hrLatest=runningHrSeries.at(-1)?.heart_rate||metrics.running_hr_7d;switch(key){case'hrv':return{key,title:'HRV',value:hrvLatest===null||hrvLatest===undefined?'-':`${formatNumber(hrvLatest,1)} ms`,subtitle:`RÃ©f ${formatBand(metrics.hrv_reference_low,metrics.hrv_reference_high,(value)=>`${formatNumber(value,1)} ms`)}`,detail:'HRV en moyenne glissante sur 7 jours Ã  partir des mesures wellness locales. Plus la valeur remonte vers la bande haute, plus la rÃ©cupÃ©ration est gÃ©nÃ©ralement favorable.',calculation:'Moyenne glissante 7 jours sur la sÃ©rie wellness locale, avec filtrage des jours sans HRV exploitable.',provenance:'DonnÃ©es wellness locales Garmin / export normalisÃ©.',reading:'Bande basse = alerte rÃ©cupÃ©ration, bande haute = zone plus confortable. La bonne lecture se fait avec le sommeil et la FC repos.',referenceNotes:[`RÃ©f basse: ${formatBand(metrics.hrv_reference_low,metrics.hrv_reference_high,(value)=>`${formatNumber(value,1)} ms`)}`,'Comparer avec la FC repos et la charge 7j/28j avant de conclure.'],chartType:'single',series:hrvSeries.map((row)=>({value:row.hrv_ms,label:formatShortDateLabel(row.metric_date),detail:formatDateLabel(row.metric_date)})).filter((row)=>Number.isFinite(Number(row.value))),tickFormatter:(value)=>`${formatNumber(value,1)} ms`,stroke:'#a38bff',fill:'rgba(163, 139, 255, 0.18)' };case'zones-all':return{key,title:'Zones FC globales',value:`${formatNumber((zonesAll.total_seconds||0)/3600,1)} h`,subtitle:'Toutes activitÃ©s',detail:'RÃ©partition approximative du temps passÃ© en zones FC sur toutes les activitÃ©s indexÃ©es. La part de chaque zone est pondÃ©rÃ©e par la durÃ©e de lâ€™activitÃ© et la FC moyenne dÃ©tectÃ©e.',calculation:'DurÃ©e des activitÃ©s pondÃ©rÃ©e par la FC moyenne de chaque activitÃ©.',provenance:'ActivitÃ©s locales Garmin + seuils de zones issus du profil de zones enregistrÃ©.',reading:'Les zones basses doivent dominer la base. Les zones hautes doivent rester des épisodes intentionnels, sinon la charge cardio dérive trop vite.',referenceNotes:['Lecture globale des zones cardio sur le mix complet des sports.','Les fractions trÃ¨s courtes ou les activitÃ©s incomplÃ¨tes sont sous-pondÃ©rÃ©es.'],chartType:'zones',zonesAll, zonesRunning, note:'Vue globale sur toutes les activitÃ©s.'};case'zones-running':return{key,title:'Zones FC running',value:`${formatNumber((zonesRunning.total_seconds||0)/3600,1)} h`,subtitle:'Running uniquement',detail:'MÃªme logique que la vue globale, mais filtrÃ©e sur le running. Câ€™est la vue Ã  privilÃ©gier pour discuter de la charge cardio utile au coach.',calculation:'DurÃ©e des activitÃ©s running pondÃ©rÃ©e par la FC moyenne de chaque activitÃ©.',provenance:'ActivitÃ©s running locales Garmin + zones cardio dÃ©tectÃ©es dans le profil utilisateur.',reading:'Cette vue permet dâ€™Ã©viter que le vÃ©lo ou la muscu masquent la lecture cardio running.',referenceNotes:['Comparer cette vue Ã  la vue globale pour mesurer le poids rÃ©el du running.'],chartType:'zones',zonesAll, zonesRunning, note:'Vue running uniquement.'};case'pace-cadence-hr':return{key,title:'Pace / cadence / FC',value:`${paceLatest?formatPace(paceLatest):'-'} Â· ${cadenceLatest===null||cadenceLatest===undefined?'-':`${formatNumber(cadenceLatest,0)} spm`} Â· ${hrLatest===null||hrLatest===undefined?'-':formatHeartRate(hrLatest)}`,subtitle:`Triptyque ${dashboardWindowDetail(state.dashboardWindowDays)} Â· ${paceCurveDebug.ready?`${paceCurveDebug.curve_points} points courbe`:'diagnostic requis'}`,detail:`Triptyque aligné sur ${dashboardWindowLabel(state.dashboardWindowDays).toLowerCase()} pour voir si l'allure, la cadence et la FC évoluent ensemble. ${paceCurveDebug.ready ? 'La modal donne la lecture complète avec axes et repères.' : `Courbe pace / FC indisponible: ${curveGap}`}`,calculation:`Séries journalières running + cadence + FC, alignées sur ${dashboardWindowDetail(state.dashboardWindowDays).toLowerCase()} et affichées en trois panneaux synchronisés.`,provenance:'ActivitÃ©s running, cadence issue des activitÃ©s normalisÃ©es, FC issue des rÃ©sumÃ©s running.',reading:'Lâ€™objectif est de voir la cadence monter vers 170 spm sans explosion de FC Ã  allure donnÃ©e.',referenceNotes:[`Cadence cible: ${cadenceTarget} spm (bande ${cadenceRefLow}-${cadenceRefHigh} spm).`,`Regarder la cohÃ©rence des trois panneaux plutÃ´t quâ€™une valeur isolÃ©e.`],chartType:'triptych',paceSeries:runningPaceSeries.map((row)=>({value:row.pace_min_per_km,label:formatShortDateLabel(row.metric_date),detail:formatDateLabel(row.metric_date)})).filter((row)=>Number.isFinite(Number(row.value))),cadenceSeries:cadenceSeries.map((row)=>({value:row.cadence_spm,label:formatShortDateLabel(row.metric_date),detail:formatDateLabel(row.metric_date)})).filter((row)=>Number.isFinite(Number(row.value))),hrSeries:runningHrSeries.map((row)=>({value:row.heart_rate,label:formatShortDateLabel(row.metric_date),detail:formatDateLabel(row.metric_date)})).filter((row)=>Number.isFinite(Number(row.value))),xTickFormatter:(item)=>item.label||'',paceReferenceLines:[{value:paceCurve[0]?.pace_min_per_km,label:'allure lente',color:'#8ae6c0'},{value:paceCurve.at(-1)?.pace_min_per_km,label:'allure rapide',color:'#ffbe55'}].filter((item)=>Number.isFinite(Number(item.value))),cadenceReferenceLines:[{value:cadenceRefLow,label:`bande basse ${formatNumber(cadenceRefLow,0)}` ,color:'#8ab4ff'},{value:cadenceTarget,label:`cible ${cadenceTarget}`,color:'#8ae6c0'},{value:cadenceRefHigh,label:`bande haute ${formatNumber(cadenceRefHigh,0)}`,color:'#8ab4ff'}].filter((item)=>Number.isFinite(Number(item.value))),hrReferenceBands:[{low:metrics.resting_hr_reference_low,high:metrics.resting_hr_reference_high,color:'#8ab4ff',label:'FC repos'}]};default:return null;}}
...
function renderMonotonePaceCurve(container,curve,options={}){
  ...
  if(values.length<3){
    ...
    container.innerHTML=`<div class="chart-shell"><p class="status-line">Courbe indisponible</p><div class="detail-block"><h4>Pourquoi</h4><ul class="detail-list">${reasons.map((reason)=>`<li>${safeChartText(reason)}</li>`).join('')}</ul></div><div class="detail-block"><h4>Lecture</h4><p>On attend au minimum ${diagnostics.stability_threshold?.min_points || 3} sorties running stables avec allure et FC exploitables. Les fractions très courtes, les échauffements et les retours au calme sont sous-pondérés.</p></div></div>`;
    return;
  }
  const width=options.width||360;
...
function renderSidebarStatus(){
  ...
  [
    ...
    {label:analysis.available?'Analyse: prête':'Analyse: en attente',warn:!analysis.available},
...
function dashboardCardSpecs(payload){
  ...
  const paceCurveDebug = metrics.pace_hr_curve_debug || {};
  ...
  return [
    ...
    { key:'sleep', title:'Sommeil', value:metrics.sleep_hours_7d === null || metrics.sleep_hours_7d === undefined ? '-' : `${formatNumber(metrics.sleep_hours_7d,1)} h`, subtitle:sleepRef !== '-' ? `RÃ©f ${sleepRef}` : 'Sommeil 7j', detail:'Le sommeil rÃ©cent correspond Ã  la moyenne des nuits sur 7 jours. Le but nâ€™est pas dâ€™avoir une valeur parfaite, mais un niveau compatible avec la charge du moment.', signals:[{label:'RÃ©cupÃ©ration',value:metrics.overreaching_flag ? 'Ã€ surveiller' : 'Correcte'},{label:'DerniÃ¨re activitÃ©',value:importStatus.latest_activity_day ? formatDateLabel(importStatus.latest_activity_day) : '-'}], chartType:'single', series:sleepChartSeries, tickFormatter:(value)=>`${formatNumber(value,1)} h`, stroke:'#8ae6c0', fill:'rgba(138, 230, 192, 0.2)', referenceLines:[{value:metrics.sleep_reference_low,label:`bande basse ${formatNumber(metrics.sleep_reference_low,1)} h`,color:'#8ab4ff'},{value:metrics.sleep_reference_high,label:`bande haute ${formatNumber(metrics.sleep_reference_high,1)} h`,color:'#8ab4ff'}].filter((item)=>Number.isFinite(Number(item.value))), referenceBands:[{low:metrics.sleep_reference_low,high:metrics.sleep_reference_high,color:'rgba(138, 230, 192, 0.12)'}].filter((band)=>Number.isFinite(Number(band.low))&&Number.isFinite(Number(band.high))) },
    { key:'resting-hr', title:'FC repos', value:formatHeartRate(metrics.resting_hr_7d), subtitle:restingRef !== '-' ? `RÃ©f ${restingRef}` : 'FC repos 7j', detail:'La FC repos rÃ©cente complÃ¨te la lecture rÃ©cupÃ©ration / stress. Si elle monte en mÃªme temps que la charge, le coach doit rester prudent.', signals:[{label:'HRV 7j',value:metrics.hrv_7d === null || metrics.hrv_7d === undefined ? '-' : formatNumber(metrics.hrv_7d,1)},{label:'Fatigue',value:metrics.fatigue_flag ? 'Oui' : 'Non'}], chartType:'single', series:restingHrChartSeries, tickFormatter:(value)=>`${formatNumber(value,0)} bpm`, stroke:'#8ab4ff', fill:'rgba(138, 180, 255, 0.2)', referenceLines:[{value:metrics.resting_hr_reference_low,label:`bande basse ${formatNumber(metrics.resting_hr_reference_low,0)} bpm`,color:'#8ab4ff'},{value:metrics.resting_hr_reference_high,label:`bande haute ${formatNumber(metrics.resting_hr_reference_high,0)} bpm`,color:'#8ab4ff'}].filter((item)=>Number.isFinite(Number(item.value))), referenceBands:[{low:metrics.resting_hr_reference_low,high:metrics.resting_hr_reference_high,color:'rgba(138, 180, 255, 0.12)'}].filter((band)=>Number.isFinite(Number(band.low))&&Number.isFinite(Number(band.high))) },
    { key:'pace-hr', title:'Pace / FC', value:paceCurveLast ? `${formatPace(paceCurveLast.pace_min_per_km)} Â· ${formatHeartRate(paceCurveLast.heart_rate)}` : '-', subtitle:`Courbe monotone Â· ${paceCurve.length} points utiles`, detail:'La courbe conserve les points utiles des derniÃ¨res sorties, puis les lisse pour rester monotone. Plus lâ€™allure devient soutenue, plus la FC doit monter.', signals:[{label:'Premier point',value:paceCurveFirst ? `${formatPace(paceCurveFirst.pace_min_per_km)} Â· ${formatHeartRate(paceCurveFirst.heart_rate)}` : '-'},{label:'Benchmark retenu',value:benchmarkSignal}], chartType:'curve', curve:paceCurve, maxHrEstimate:metrics.max_hr_estimate, diagnostics:paceCurveDebug },
    { key:'cadence', title:'Cadence', value:metrics.cadence_7d === null || metrics.cadence_7d === undefined ? '-' : `${formatNumber(metrics.cadence_7d,0)} spm`, subtitle:cadenceSubtitle, detail:'La cadence suit la qualitÃ© du geste et sa rÃ©gularitÃ© sur les derniÃ¨res sorties. On la lit avec lâ€™allure et le volume, pour voir si la foulÃ©e Ã©volue vraiment.', signals:[{label:'28 jours',value:metrics.cadence_28d === null || metrics.cadence_28d === undefined ? '-' : `${formatNumber(metrics.cadence_28d,0)} spm`},{label:'RÃ©fÃ©rence',value:cadenceRef}], chartType:'single', series:cadenceValues, tone:'primary', referenceLines:[{value:cadenceRefLow,label:`bande basse ${formatNumber(cadenceRefLow,0)} spm`,color:'#8ab4ff'},{value:cadenceTarget,label:`cible ${cadenceTarget} spm`,color:'#8ae6c0'},{value:cadenceRefHigh,label:`bande haute ${formatNumber(cadenceRefHigh,0)} spm`,color:'#8ab4ff'}].filter((item)=>Number.isFinite(Number(item.value))) },
  ];
}
function renderDashboardModal(card){
  ...
  if(card.chartType==='single'){
    renderSparkline(dom.dashboardModalChart,card.series||[],{id:`modal-${card.key}`,stroke:card.stroke||('#8ab4ff'),fill:card.fill||'rgba(138, 180, 255, 0.22)',width:1200,height:380,tickFormatter:card.tickFormatter,xTickCount:7,yTickCount:7,title:card.title,yLabel:card.key==='sleep'?'h':card.key==='resting-hr'?'bpm':card.key==='charge-ratio'?'ratio':'km',referenceLines:card.referenceLines||[],referenceBands:card.referenceBands||[]});
    ...
    renderMonotonePaceCurve(dom.dashboardModalChart,card.curve||[],{id:`modal-${card.key}`,width:1200,height:420,maxHrEstimate:card.maxHrEstimate,diagnostics:card.diagnostics,xTickCount:8,yTickCount:7});
    ...
    renderTriptychChart(dom.dashboardModalChart,card.paceSeries||[],card.cadenceSeries||[],card.hrSeries||[],{id:`modal-${card.key}`,labels:{pace:'Allure',cadence:'Cadence',hr:'FC'},xTickFormatter:(item)=>item.label||'',width:1200,height:180,xTickCount:8});
  }else{
...
function renderDashboardSection(){
  ...
  const sleepSeries=Array.isArray(trend.daily_sleep)?trend.daily_sleep:[];
  const restingHrSeries=Array.isArray(trend.daily_resting_hr)?trend.daily_resting_hr:[];
  const hrvSeries=Array.isArray(trend.daily_hrv)?trend.daily_hrv:[];
  const runningPaceSeries=Array.isArray(trend.daily_running_pace)?trend.daily_running_pace:[];
  const runningHrSeries=Array.isArray(trend.daily_running_hr)?trend.daily_running_hr:[];
  ...
  const runningPaceChartSeries=runningPaceSeries.map((row)=>({value:row.pace_min_per_km,label:formatShortDateLabel(row.metric_date),detail:formatDateLabel(row.metric_date)})).filter((row)=>Number.isFinite(Number(row.value)));
  const runningHrChartSeries=runningHrSeries.map((row)=>({value:row.heart_rate,label:formatShortDateLabel(row.metric_date),detail:formatDateLabel(row.metric_date)})).filter((row)=>Number.isFinite(Number(row.value)));
  const cadenceChartSeries=Array.isArray(trend.cadence_daily)?trend.cadence_daily.map((row)=>({value:row.cadence_spm,label:formatShortDateLabel(row.metric_date),detail:formatDateLabel(row.metric_date)})).filter((row)=>Number.isFinite(Number(row.value))):[];const windowText=dashboardWindowDetail(state.dashboardWindowDays);['volume-trend-window','bike-volume-trend-window','load-ratio-trend-window','sleep-trend-window','resting-hr-trend-window','hrv-trend-window','pace-hr-trend-window','cadence-trend-window','pace-cadence-hr-trend-window'].forEach((id)=>{const node=document.getElementById(id);if(node)node.textContent=windowText});
  ...
  dom.analysisSubtitle.textContent=metrics.cadence_target_spm?`Cible ${formatNumber(metrics.cadence_target_spm,0)} spm`:(metrics.cadence_reference_low!==null&&metrics.cadence_reference_low!==undefined&&metrics.cadence_reference_high!==null&&metrics.cadence_reference_high!==undefined?`RÃ©f ${formatNumber(metrics.cadence_reference_low,0)}-${formatNumber(metrics.cadence_reference_high,0)} spm`:'Cadence sans rÃ©fÃ©rence');
  ...
  if(dom.bikeVolumeTrend) renderSparkline(dom.bikeVolumeTrend,bikeSeries.map((row)=>({value:row.distance_km||0,label:formatShortDateLabel(row.metric_date),detail:formatDateLabel(row.metric_date)})),{id:'bike-volume',stroke:'#8ab4ff',fill:'rgba(138, 180, 255, 0.26)',height:128,width:360,title:'Volume vÃ©lo',yLabel:'km'});
  ...
  if(dom.hrvTrend) renderSparkline(dom.hrvTrend,hrvSeries.map((row)=>({value:row.hrv_ms,label:formatShortDateLabel(row.metric_date),detail:formatDateLabel(row.metric_date)})).filter((row)=>Number.isFinite(Number(row.value))),{id:'hrv',stroke:'#a38bff',fill:'rgba(163, 139, 255, 0.2)',height:128,width:360,title:'HRV',yLabel:'ms',tickFormatter:(value)=>`${formatNumber(value,1)} ms`,referenceBands:[{low:metrics.hrv_reference_low,high:metrics.hrv_reference_high,color:'rgba(163, 139, 255, 0.12)'}]});
  const paceSummary=Array.isArray(trend.pace_hr_curve)&&trend.pace_hr_curve.length?`${formatPace(trend.pace_hr_curve[0].pace_min_per_km)} â†’ ${formatPace(trend.pace_hr_curve.at(-1).pace_min_per_km)} Â· ${trend.pace_hr_curve.length} points`:'Pas encore de courbe';
  dom.paceHrTrendSummary.textContent=paceSummary;
  renderMonotonePaceCurve(dom.paceHrTrend,trend.pace_hr_curve||[],{id:'pace-hr',width:360,height:240,maxHrEstimate:metrics.max_hr_estimate,diagnostics:paceCurveDebug});
...
function renderCoachSection(){
  ...
  const items=[importStatus.available?'Données locales prêtes':'Pas de données locales',analysis.available?'Analyse locale prête':'Analyse locale partielle',`Provider: ${providerText}`];
...
function buildDiagnosticsSummary(){const payload=currentPayload();const importStatus=currentImportStatus();const provider=payload.provider||{};return[`build=${APP_VERSION}`,`bootId=${state.bootId}`,`bootTraceEvents=${state.bootTrace.length}`,`bootErrors=${Array.isArray(state.bootErrors)?state.bootErrors.length:0}`,`url=${window.location.href}`,`hash=${window.location.hash||'#import'}`,`activeSection=${state.activeSection}`,`workspace=${state.workspace}`,`provider=${state.provider}`,`providerStatus=${provider.status||'unknown'}`,`dataAvailable=${importStatus.available?'yes':'no'}`,`latestDay=${importStatus.latest_activity_day||payload.latest_day||'-'}`,`swState=${runtimeInfo.serviceWorkerState}`,`swController=${runtimeInfo.serviceWorkerController?'yes':'no'}`,`swScope=${runtimeInfo.serviceWorkerScope}`,`online=${navigator.onLine?'yes':'no'}`].join('\n')}
...
function renderAllSections(){safeBootStep('renderSectionVisibility',renderSectionVisibility);safeBootStep('renderSidebarStatus',renderSidebarStatus);safeBootStep('renderSectionHeader',renderSectionHeader);safeBootStep('renderImportSection',renderImportSection);safeBootStep('renderDashboardSection',renderDashboardSection);safeBootStep('renderCoachSection',renderCoachSection);safeBootStep('renderTerminalSection',renderTerminalSection);safeBootStep('renderSettingsSection',renderSettingsSection)}
