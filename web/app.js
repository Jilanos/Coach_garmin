...
function dashboardCardSpecs(payload){
  ...
  return [
    ...
    {key:'charge-ratio',title:'Charge relative',value:metrics.load_ratio_7_28===null||metrics.load_ratio_7_28===undefined?'-':formatNumber(metrics.load_ratio_7_28,2),subtitle:`Réf ${ratioRef}`,detail:'Le ratio compare la charge 7 jours à la charge 28 jours.',calculation:'Calcul = load_7d / load_28d. La ligne 1.00 correspond à un équilibre entre la charge courte et la charge plus installée.',provenance:'Table `derived_daily_metrics`, calculée localement à partir des activités Garmin normalisées et des charges dérivées.',reading:'Autour de 1.00, la dynamique est stable. Sous la bande basse, on est en retrait. Au-dessus de la bande haute, on surveille la fatigue et l’overreaching.',referenceNotes:[`Bande de lecture actuelle ${ratioRef}.`,'Référence pratique proche de l’Acute:Chronic Workload Ratio, ici implémentée localement en 7j/28j.'],signals:[{label:'Fatigue',value:metrics.fatigue_flag?'Oui':'Non'},{label:'Overreaching',value:metrics.overreaching_flag?'Oui':'Non'}],chartType:'single',series:loadRatioChartSeries,tickFormatter:(value)=>formatNumber(value,2),stroke:'#ffbe55',fill:'rgba(255, 190, 85, 0.22)',referenceLines:[{value:ratioRefLow,label:`bande basse ${formatNumber(ratioRefLow,2)}`,color:'#8ab4ff'},{value:1.0,label:'équilibre 1.00',color:'#8ae6c0'},{value:ratioRefHigh,label:`bande haute ${formatNumber(ratioRefHigh,2)}`,color:'#8ab4ff'}].filter((item)=>Number.isFinite(Number(item.value))),referenceBands:[{low:ratioRefLow,high:ratioRefHigh,color:'rgba(255, 190, 85, 0.14)'}].filter((band)=>Number.isFinite(Number(band.low))&&Number.isFinite(Number(band.high)))},
    {key:'sleep',title:'Sommeil',value:sleepLast?`${formatNumber(sleepLast.value,1)} h`:'-',subtitle:'Valeur brute journalière / sans lissage par défaut',detail:'Le sommeil affiche la valeur journalière brute. Le lissage est retiré par défaut pour ne pas masquer la variabilité réelle.',calculation:'Agrégation journalière brute depuis `wellness_daily` avec conversion des secondes en heures.',provenance:'Mesures wellness Garmin normalisées, lues depuis `wellness_daily`.',reading:'Lire la succession des nuits réellement observées, puis croiser avec la FC repos et la HRV.',referenceNotes:['Valeur brute journalière, sans moyenne 7 jours par défaut.','Les références restent des bandes de lecture, pas un objectif absolu.'],signals:[{label:'Dernière nuit',value:sleepLast?.detail||'-'},{label:'Points visibles',value:String(sleepChartSeries.length)}],chartType:'single',series:sleepChartSeries,tickFormatter:(value)=>`${formatNumber(value,1)} h`,stroke:'#8ae6c0',fill:'rgba(138, 230, 192, 0.2)',yLabel:'h',drawArea:false,referenceLines:[{value:metrics.sleep_reference_low,label:`bande basse ${formatNumber(metrics.sleep_reference_low,1)} h`,color:'#8ab4ff'},{value:metrics.sleep_reference_high,label:`bande haute ${formatNumber(metrics.sleep_reference_high,1)} h`,color:'#8ab4ff'}].filter((item)=>Number.isFinite(Number(item.value))),referenceBands:[{low:metrics.sleep_reference_low,high:metrics.sleep_reference_high,color:'rgba(138, 230, 192, 0.12)'}].filter((band)=>Number.isFinite(Number(band.low))&&Number.isFinite(Number(band.high)))},
    ...
    {key:'hrv',title:'HRV',value:hrvLast?`${formatNumber(hrvLast.value,1)} ms`:'-',subtitle:'Valeur brute journalière / sans lissage par défaut',detail:'La HRV affiche la mesure journalière brute. Les accents et références sont corrigés au niveau source.',calculation:'Agrégation journalière brute depuis `wellness_daily`.',provenance:'Mesures wellness Garmin normalisées, lues depuis `wellness_daily`.',reading:'Observer la variabilité réelle et ses ruptures, sans moyenne glissante par défaut. Une baisse persistante combinée à une FC repos plus haute et un sommeil plus bas peut signaler une récupération moins bonne.',referenceNotes:['Valeur brute journalière, sans lissage par défaut.','La lecture de HRV se fait toujours avec le contexte de charge, de sommeil et de FC repos.'],signals:[{label:'Dernière mesure',value:hrvLast?.detail||'-'},{label:'Points visibles',value:String(hrvChartSeries.length)}],chartType:'single',series:hrvChartSeries,tickFormatter:(value)=>`${formatNumber(value,1)} ms`,stroke:'#a38bff',fill:'rgba(163, 139, 255, 0.16)',yLabel:'ms',drawArea:false,referenceBands:[{low:metrics.hrv_reference_low,high:metrics.hrv_reference_high,color:'rgba(163, 139, 255, 0.12)'}].filter((band)=>Number.isFinite(Number(band.low))&&Number.isFinite(Number(band.high)))},
...
function renderDashboardSection(){
  ...
  const cadenceBounds=cadenceSummary.plot_bounds?.min!==null&&cadenceSummary.plot_bounds?.min!==undefined&&cadenceSummary.plot_bounds?.max!==null&&cadenceSummary.plot_bounds?.max!==undefined?`${formatNumber(cadenceSummary.plot_bounds.min,1)}-${formatNumber(cadenceSummary.plot_bounds.max,1)} spm`:'non calculées';
  ['volume-trend-window','bike-volume-trend-window','load-ratio-trend-window','sleep-trend-window','resting-hr-trend-window','hrv-trend-window','pace-hr-trend-window','cadence-trend-window','pace-cadence-hr-trend-window'].forEach((id)=>{const node=document.getElementById(id);if(node)node.textContent=windowText});
  ...
  if(dom.sleepTrendSummary) dom.sleepTrendSummary.textContent=lastSleep?`${formatNumber(lastSleep.sleep_hours||lastSleep.value,1)} h / brut`:'Pas encore de sommeil';
  ...
  if(dom.hrvTrendSummary) dom.hrvTrendSummary.textContent=lastHrv?`${formatNumber(lastHrv.hrv_ms||lastHrv.value,1)} ms / brut`:'Pas encore de HRV';
